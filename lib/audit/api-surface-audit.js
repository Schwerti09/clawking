...
function detectGuards(content) {
  const markers = {
    ...
    sharedSecret: /isAuthorizedBySharedSecret|CRON_SECRET|GEO_[A-Z0-9_]*SECRET|_SECRET/.test(content),
    apiKeyAuth: /validateApiKey|X-API-Key|INTEL_API_KEYS|Authorization:\s*Bearer/.test(content),
