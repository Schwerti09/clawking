...
export default function Tools() {
  ...
  const [rbPrompt, setRbPrompt] = useState("")
  ...
  async function generateRunbook() {
    ...
    try {
      ...
      if (!res.ok) {
        setRbErr(data?.error || "AI request failed")
        return
      }
  ...
  return (
    <Container>
      <div className="py-16 max-w-6xl mx-auto">
        ...
        <div className="mt-8 p-7 rounded-2xl border border-gray-800 bg-black/30">
          <h2 className="text-2xl font-black mb-2">Runbook Factory (KI)</h2>
          <p className="text-gray-400 mb-4">
            Beschreibe dein Problem (Logs, Ziel, Umgebung). Wir generieren ein Runbook mit Steps, Verify & Rollback.
          </p>

          <textarea
            className="w-full h-40 rounded-xl bg-black/40 border border-gray-700 p-4 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-brand-cyan/30 focus:border-brand-cyan"
            placeholder="z.B. 'Next.js App auf Netlify: Stripe Webhook 500, brauche Debug-Runbook…'"
            value={rbPrompt}
            onChange={(e) => setRbPrompt(e.target.value)}
          />

          <div className="mt-4 flex flex-wrap gap-3 items-center">
            <button
              onClick={generateRunbook}
              disabled={!rbPrompt.trim() || rbLoading}
              className="px-5 py-3 rounded-xl bg-brand-cyan/15 border border-brand-cyan/30 hover:bg-brand-cyan/25 disabled:opacity-50"
            >
              {rbLoading ? "Generiere…" : "Runbook generieren"}
            </button>
            {rbErr ? <div className="text-sm text-brand-red">{rbErr}</div> : null}
          </div>

          {rbOut ? (
            <pre className="mt-5 whitespace-pre-wrap rounded-xl bg-black/40 border border-gray-800 p-4 text-sm text-gray-200">
              {rbOut}
            </pre>
          ) : null}
        </div>
