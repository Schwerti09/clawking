// China Mega Expansion Status - GET based approach
import { NextRequest, NextResponse } from "next/server"
import { dbQuery } from "@/lib/db"

export async function GET(request: NextRequest) {
  try {
    console.log("🔍 Checking China Mega Expansion status...")
    
    // Check if China cities exist
    const checkQuery = `
      SELECT c.city_slug, c.name_de, c.name_en, c.claw_score, c.title, 
             q.quality_score, q.content_depth, q.local_relevance, q.technical_accuracy
      FROM geo_cities c
    ...
    const cities = await dbQuery(checkQuery)
    
    // If cities don't exist, create them
    if (cities.length === 0) {
      console.log("🚀 China cities not found, creating...")
      ...
      for (const city of CHINA_CITIES) {
        console.log(`🔧 Creating ${city.city_slug}...`)
      ...
      const newCities = await dbQuery(checkQuery)
      
      return NextResponse.json({
        success: true,
        message: "China Mega Expansion completed via GET",
        cities: newCities.map(c => ({
          ...c,
          overall_quality: Math.min(c.quality_score || 0, c.content_depth || 0, c.local_relevance || 0, c.technical_accuracy || 0),
          status: (Math.min(c.quality_score || 0, c.content_depth || 0, c.local_relevance || 0, c.technical_accuracy || 0) >= 85) ? "✅ READY" : "⚠️ NEEDS WORK"
        })),
        summary: {
          total_cities: newCities.length,
          ready_for_traffic: newCities.filter(c => Math.min(c.quality_score || 0, c.content_depth || 0, c.local_relevance || 0, c.technical_accuracy || 0) >= 85).length,
          needs_work: newCities.filter(c => Math.min(c.quality_score || 0, c.content_depth || 0, c.local_relevance || 0, c.technical_accuracy || 0) < 85).length
        }
      })
    }
    
    // Cities already exist, return status
    return NextResponse.json({
      success: true,
      message: "China Mega Expansion status check",
      cities: cities.map(c => ({
        ...c,
        overall_quality: Math.min(c.quality_score || 0, c.content_depth || 0, c.local_relevance || 0, c.technical_accuracy || 0),
        status: (Math.min(c.quality_score || 0, c.content_depth || 0, c.local_relevance || 0, c.technical_accuracy || 0) >= 85) ? "✅ READY" : "⚠️ NEEDS WORK"
      ...
      summary: {
        ...
        ready_for_traffic: cities.filter(c => Math.min(c.quality_score || 0, c.content_depth || 0, c.local_relevance || 0, c.technical_accuracy || 0) >= 85).length,
        needs_work: cities.filter(c => Math.min(c.quality_score || 0, c.content_depth || 0, c.local_relevance || 0, c.technical_accuracy || 0) < 85).length
      }
    })

  } catch (error) {
    console.error("Error in China status:", error)
    return NextResponse.json(
