...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const title = isDE
    ? "EU AI Act Compliance für Self-Hosted KI-Systeme 2025/2026 | ClawGuru"
    : "EU AI Act Compliance for Self-Hosted AI Systems 2025/2026 | ClawGuru"
  const description = isDE
    ? "EU AI Act Compliance umsetzen: Risikoklassifizierung, High-Risk-Anforderungen, technische Dokumentation, Konformitätsbewertung und Post-Market-Monitoring für selbst gehostete KI-Systeme."
    : "Implement EU AI Act compliance: risk classification, high-risk requirements, technical documentation, conformity assessment and post-market monitoring for self-hosted AI systems."
  return {
    title, description,
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <h1 className="text-4xl font-bold mb-4 text-gray-100">
          {isDE ? "EU AI Act Compliance für Self-Hosted KI-Systeme" : "EU AI Act Compliance for Self-Hosted AI Systems"}
        </h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            { value: "4", label: isDE ? "Risikoklassen" : "Risk classes" },
            { value: "Aug 2026", label: isDE ? "High-Risk Deadline" : "High-risk deadline" },
            { value: "7", label: isDE ? "High-Risk Anforderungen" : "High-risk requirements" },
            { value: "4/7", label: isDE ? "Mit Moltbot automatisiert" : "Automated with Moltbot" },
          ].map((s) => (
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">
            {isDE ? "4 Risikoklassen — Klassifizierung" : "4 Risk Classes — Classification"}
          </h2>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">
            {isDE ? "High-Risk: 7 Pflichtanforderungen" : "High-Risk: 7 Mandatory Requirements"}
          </h2>
              ...
              <div key={req.id} className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className={`text-xs font-bold px-2 py-0.5 rounded ${req.auto ? "bg-green-900 text-green-300" : "bg-gray-700 text-gray-400"}`}>
                    {req.auto ? (isDE ? "Automatisierbar" : "Automatable") : (isDE ? "Manuell" : "Manual")}
                  </span>
                </div>
        ...
        <section className="mb-10">
          ...
          <div className="space-y-3">
            {[
              { date: "Aug 2024", e: isDE ? "EU AI Act in Kraft getreten" : "EU AI Act entered into force", done: true },
              { date: "Feb 2025", e: isDE ? "Verbotene KI-Systeme (Art. 5) — JETZT AKTIV" : "Prohibited AI systems (Art. 5) — NOW ACTIVE", done: true },
              { date: "Aug 2025", e: isDE ? "GPAI-Modell-Anforderungen (GPT-4-Klasse)" : "GPAI model obligations (GPT-4 class)", done: false },
              { date: "Aug 2026", e: isDE ? "High-Risk KI-Anforderungen vollständig anwendbar" : "High-risk AI requirements fully applicable", done: false },
              { date: "Aug 2027", e: isDE ? "High-Risk KI in regulierten Produkten (Medizinprodukte, Maschinen)" : "High-risk AI in regulated products (medical devices, machinery)", done: false },
            ].map((t) => (
              <div key={t.date} className={`flex items-start gap-4 p-3 rounded-lg border ${t.done ? "bg-green-900/30 border-green-700" : "bg-gray-800 border-gray-700"}`}>
                ...
                {t.done && <span className="text-xs text-green-400 flex-shrink-0 ml-auto">✓ {isDE ? "Aktiv" : "Active"}</span>}
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            <a href={`/${locale}/moltbot/ai-agent-security`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "Art. 15 — Cybersecurity für KI" : "Art. 15 — Cybersecurity for AI"}</div>
            </a>
            <a href={`/${locale}/solutions/dsgvo-compliance-automation`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "AI Act + DSGVO kombinieren" : "Combine AI Act + GDPR"}</div>
            </a>
            ...
            <a href={`/${locale}/moltbot/ai-red-teaming`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "Art. 15 — Robustheitstests" : "Art. 15 — Robustness testing"}</div>
            </a>
          </div>
        </section>
