...
export default function PciDssCompliancePage({ params }: PageProps) {
  ...
  const faqSchema = {
    ...
    '@type': 'FAQPage',
    mainEntity: [
      { '@type': 'Question', name: locale === 'de' ? 'Was sind die 12 PCI DSS Anforderungen?' : 'What are the 12 PCI DSS requirements?', acceptedAnswer: { '@type': 'Answer', text: locale === 'de' ? 'PCI DSS v4.0 hat 12 Hauptanforderungen: 1. Firewall installieren, 2. Keine Hersteller-Standardpasswörter, 3. Karteninhaberdaten schützen, 4. Verschlüsselung bei Übertragung, 5. Antivirus, 6. Sichere Systeme entwickeln, 7. Zugriff einschränken, 8. Authentifizierung, 9. Physischer Zugang, 10. Logging & Monitoring, 11. Sicherheitstests, 12. Informationssicherheitsrichtlinie.' : 'PCI DSS v4.0 has 12 main requirements: 1. Install firewall, 2. No vendor default passwords, 3. Protect cardholder data, 4. Encrypt transmission, 5. Antivirus, 6. Develop secure systems, 7. Restrict access, 8. Authentication, 9. Physical access, 10. Logging & monitoring, 11. Security tests, 12. Information security policy.' } },
      { '@type': 'Question', name: locale === 'de' ? 'Was ist der Unterschied zwischen PCI DSS SAQ und QSA-Audit?' : 'What is the difference between PCI DSS SAQ and QSA audit?', acceptedAnswer: { '@type': 'Answer', text: locale === 'de' ? 'Self-Assessment Questionnaire (SAQ): Für kleinere Händler (SAQ A bis D). SAQ A: Nur Redirect/iFrame-Zahlungen, einfachster SAQ. SAQ D: Vollständige Karteninhaberdaten-Speicherung, komplexester. QSA (Qualified Security Assessor): Pflicht für Level 1 Merchants (> 6 Mio. Transaktionen/Jahr). Unabhängiger Auditor prüft alle 12 Anforderungen vor Ort.' : 'Self-Assessment Questionnaire (SAQ): for smaller merchants (SAQ A through D). SAQ A: redirect/iFrame payments only, simplest SAQ. SAQ D: full cardholder data storage, most complex. QSA (Qualified Security Assessor): mandatory for Level 1 merchants (> 6M transactions/year). Independent auditor validates all 12 requirements on-site.' } },
      { '@type': 'Question', name: locale === 'de' ? 'Wie reduziere ich den PCI DSS Scope?' : 'How do I reduce the PCI DSS scope?', acceptedAnswer: { '@type': 'Answer', text: locale === 'de' ? 'PCI Scope-Reduktion: Tokenisierung (Kartendaten durch Token ersetzen, nie selbst speichern). Point-to-Point Encryption (P2PE): verschlüsselt Karteninhaberdaten vom Eingabegerät. Hosted Payment Page / iFrame: Zahlung vollständig an PSP (Stripe, Braintree) auslagern. Ziel: SAQ A Niveau erreichen (minimalster Scope). Netzwerksegmentierung isoliert CDE.' : 'PCI scope reduction: tokenization (replace card data with token, never store yourself). Point-to-Point Encryption (P2PE): encrypts cardholder data from input device. Hosted payment page / iFrame: fully outsource payment to PSP (Stripe, Braintree). Goal: reach SAQ A level (minimal scope). Network segmentation isolates CDE.' } },
      { '@type': 'Question', name: locale === 'de' ? 'Was sind die PCI DSS Penetration Testing Anforderungen?' : 'What are the PCI DSS penetration testing requirements?', acceptedAnswer: { '@type': 'Answer', text: locale === 'de' ? 'PCI DSS Requirement 11.4: jährlicher Penetrationstest (extern und intern). Nach signifikanten Änderungen wiederholen. Pentest muss CDE (Cardholder Data Environment) und Segmentierung prüfen. Scope-Validierung: Segmentierungstest nachweisen dass Out-of-scope-Netzwerke wirklich isoliert sind. Qualifizierter Tester (OSCP, QSA) für Level 1.' : 'PCI DSS Requirement 11.4: annual penetration test (external and internal). Repeat after significant changes. Pentest must test CDE (Cardholder Data Environment) and segmentation. Scope validation: segmentation test proving out-of-scope networks are truly isolated. Qualified tester (OSCP, QSA) for Level 1.' } },
    ],
  }
