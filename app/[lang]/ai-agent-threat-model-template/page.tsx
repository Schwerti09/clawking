...
import { getAiThreatModelCopy } from "@/lib/content-ai-agent-threat-model-i18n"
...
export async function generateMetadata(props: { params: { lang: string } }): Promise<Metadata> {
  ...
  const copy = getAiThreatModelCopy(locale)
  return {
    title: copy.title,
    description: copy.description,
    alternates: buildLocalizedAlternates(locale, "/ai-agent-threat-model-template"),
    openGraph: {
      title: copy.title,
      description: copy.description,
      type: "article",
...
export default function AiThreatModelTemplatePage(props: { params: { lang: string } }) {
  const locale = (SUPPORTED_LOCALES.includes(props.params.lang as Locale) ? props.params.lang : "de") as Locale
  const copy = getAiThreatModelCopy(locale)
  ...
  const articleJsonLd = {
    ...
    description: copy.description,
    inLanguage: locale,
    mainEntityOfPage: `${prefix}/ai-agent-threat-model-template`,
  }
      ...
      <Container>
        <article className="mx-auto max-w-4xl space-y-8">
          <header className="space-y-4">
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-cyan-400/90">Threat Model Template</p>
          ...
          <section className="rounded-2xl border border-white/10 bg-black/20 p-6">
            <h2 className="text-xl font-black text-white mb-4">{copy.modelTitle}</h2>
            <ul className="space-y-2 text-zinc-300 text-sm">
              {copy.modelBlocks.map((block) => (
          ...
          <section className="rounded-2xl border border-white/10 bg-black/20 p-6">
            <h2 className="text-xl font-black text-white mb-4">{copy.promptTitle}</h2>
          ...
          <section className="rounded-2xl border border-white/10 bg-black/20 p-6">
            <h2 className="text-xl font-black text-white mb-3">{copy.validationTitle}</h2>
            <ul className="space-y-2 text-zinc-300 text-sm">
