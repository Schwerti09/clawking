...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const title = pick(isDE, "LLM Output Watermarking: LLM-Output-Watermarking | ClawGuru Moltbot", "LLM Output Watermarking: LLM Output Watermarking | ClawGuru Moltbot")
  const description = pick(isDE, "LLM-Output-Watermarking: Statistical Watermarking, Cryptographic Watermarking, Watermark Detection und Watermark Robustness für LLM-Output-Watermarking.", "LLM output watermarking: statistical watermarking, cryptographic watermarking, watermark detection and watermark robustness for LLM output watermarking.")
  return {
    title, description,
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-amber-900 border-l-4 border-amber-500 p-4 mb-8 text-sm text-amber-100">
          <strong className="text-amber-100">"Not a Pentest" Notice</strong>: {pick(isDE, "LLM-Output-Watermarking-Guide für eigene KI-Systeme.", "LLM output watermarking guide for your own AI systems.")}
        </div>
        <div className="mb-3"><span className="text-xs font-bold uppercase tracking-widest text-cyan-400">Moltbot · Batch 27</span></div>
        <h1 className="text-4xl font-bold mb-4 text-gray-100">{pick(isDE, "LLM Output Watermarking", "LLM Output Watermarking")}</h1>
        <p className="text-lg text-gray-300 mb-6">
          {pick(isDE, "AI-Content ohne Watermarking ist unsichtbar — Disinformation, Copyright-Verletzungen und Missbrauch. Vier Kontrollen: Statistical Watermarking, Cryptographic Watermarking, Watermark Detection und Watermark Robustness.", "AI content without watermarking is invisible — disinformation, copyright violations and abuse. Four controls: statistical watermarking, cryptographic watermarking, watermark detection and watermark robustness.")}
        </p>
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{pick(isDE, "4 Output-Watermarking-Kontrollen", "4 Output Watermarking Controls")}</h2>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            <a href={`/${locale}/moltbot/llm-model-watermarking`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{pick(isDE, "Model-Watermarking", "Model watermarking")}</div>
            </a>
            <a href={`/${locale}/moltbot/llm-output-validation`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{pick(isDE, "Output-Validation", "Output validation")}</div>
            </a>
