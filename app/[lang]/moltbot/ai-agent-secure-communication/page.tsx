...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const title = isDE
    ? "AI Agent Secure Communication: KI-Agenten-Secure-Communication | ClawGuru Moltbot"
    : "AI Agent Secure Communication: AI Agent Secure Communication | ClawGuru Moltbot"
  const description = isDE
    ? "KI-Agenten-Secure-Communication: TLS Encryption, Message Signing, Mutual Authentication und Secure Channel Establishment für KI-Agenten-Kommunikation."
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-amber-900 border-l-4 border-amber-500 p-4 mb-8 text-sm text-amber-100">
          <strong className="text-amber-100">"Not a Pentest" Notice</strong>: {isDE ? "Agent-Secure-Communication-Guide für eigene KI-Systeme." : "Agent secure communication guide for your own AI systems."}
        </div>
        <div className="mb-3"><span className="text-xs font-bold uppercase tracking-widest text-cyan-400">Moltbot · Batch 20</span></div>
        <h1 className="text-4xl font-bold mb-4 text-gray-100">{isDE ? "AI Agent Secure Communication" : "AI Agent Secure Communication"}</h1>
        <p className="text-lg text-gray-300 mb-6">
          {isDE
            ? "KI-Agenten ohne Secure Communication können abgehört und manipuliert werden — ohne Verschlüsselung bleiben Nachrichten ungeschützt. Vier Kontrollen: TLS Encryption, Message Signing, Mutual Authentication und Secure Channel Establishment."
            : "AI agents without secure communication can be eavesdropped on and manipulated — without encryption, messages remain unprotected. Four controls: TLS encryption, message signing, mutual authentication and secure channel establishment."}
        </p>
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{isDE ? "4 Agent-Secure-Communication-Kontrollen" : "4 Agent Secure Communication Controls"}</h2>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <a href={`/${locale}/moltbot/llm-data-encryption-in-transit`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "TLS-Overview" : "TLS overview"}</div>
            </a>
            <a href={`/${locale}/moltbot/llm-data-encryption-at-rest`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "Key-Management" : "Key management"}</div>
            </a>
            <a href={`/${locale}/moltbot/ai-agent-security`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              <div className="font-semibold text-cyan-400">AI Agent Security</div>
              <div className="text-sm text-gray-300">{isDE ? "Communication-Overview" : "Communication overview"}</div>
            </a>
          </div>
        </section>
      </div>
