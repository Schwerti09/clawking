...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = isDE
    ? "AI Agent Human Oversight: Menschliche Aufsicht über AI-Agents | ClawGuru"
    : "AI Agent Human Oversight: Human Oversight for AI Agents | ClawGuru"
  const description = isDE
    ? "AI Agent Human Oversight für Moltbot-Deployments. HITL, Approval Workflows, Escalation Patterns und Oversight Frameworks für sichere AI-Agent-Systeme. Mit Moltbot automatisierbar."
    : "AI agent human oversight for Moltbot deployments. HITL, approval workflows, escalation patterns and oversight frameworks for secure AI agent systems."
  return {
    title,
    description,
    keywords: [
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4 text-gray-100">
            {isDE ? "AI Agent Human Oversight" : "AI Agent Human Oversight"}
          </h1>
          <p className="text-lg text-gray-300 mb-4">
            {isDE
              ? "AI Agent Human Oversight für Moltbot-Deployments. HITL, Approval Workflows, Escalation Patterns und Oversight Frameworks für sichere AI-Agent-Systeme."
              : "AI agent human oversight for Moltbot deployments. HITL, approval workflows, escalation patterns and oversight frameworks for secure AI agent systems."}
          </p>
        </div>
        ...
        <section className="mb-10">
          ...
          <div className="space-y-4">
            <div className="bg-green-900 p-4 rounded-lg border border-green-700">
              <h3 className="font-semibold text-green-300 mb-2">{isDE ? "Adaptive HITL Thresholds" : "Adaptive HITL Thresholds"}</h3>
              <p className="text-sm text-green-200">{isDE ? "Dynamische Anpassung von HITL-Schwellenwerten basierend auf Agent-Performance und Risikolevel." : "Dynamic adjustment of HITL thresholds based on agent performance and risk level."}</p>
            </div>
            <div className="bg-blue-900 p-4 rounded-lg border border-blue-700">
              <h3 className="font-semibold text-blue-300 mb-2">{isDE ? "Confidence-Based Routing" : "Confidence-Based Routing"}</h3>
              <p className="text-sm text-blue-200">{isDE ? "Automatisches Routing zu menschlichen Reviewern bei niedrigem Agent-Confidence-Score." : "Automatic routing to human reviewers when agent confidence score is low."}</p>
            </div>
            <div className="bg-yellow-900 p-4 rounded-lg border border-yellow-700">
              <h3 className="font-semibold text-yellow-300 mb-2">{isDE ? "Oversight Dashboard" : "Oversight Dashboard"}</h3>
              <p className="text-sm text-yellow-200">{isDE ? "Echtzeit-Dashboard für menschliche Aufsicht. Pending approvals, agent activity und risk scores." : "Real-time dashboard for human oversight. Pending approvals, agent activity and risk scores."}</p>
            </div>
            <div className="bg-red-900 p-4 rounded-lg border border-red-700">
              <h3 className="font-semibold text-red-300 mb-2">{isDE ? "Dead Man's Switch" : "Dead Man's Switch"}</h3>
              <p className="text-sm text-red-200">{isDE ? "Automatischer Agent-Stopp wenn keine menschliche Bestätigung innerhalb eines Zeitfensters erfolgt." : "Automatic agent stop when no human confirmation occurs within a time window."}</p>
            </div>
          </div>
        </section>
        ...
        <section className="mb-10">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <a href={`/${locale}/securitycheck`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "Überprüfen Sie Ihre Infrastruktur auf Schwachstellen" : "Check your infrastructure for vulnerabilities"}</div>
            </a>
            ...
            <a href={`/${locale}/openclaw`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              <div className="font-semibold text-cyan-400">{isDE ? "OpenClaw" : "OpenClaw"}</div>
