...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = pick(isDE, "AI Agent Security Mexico City: LFPDPPP, Zero-Trust, Moltbot | ClawGuru", "AI Agent Security Mexico City: LFPDPPP, Zero-Trust, Moltbot | ClawGuru")
  const description = pick(isDE, "Mexico City Security Guide für AI-Agents: LFPDPPP-Compliance, Zero-Trust und schnelle Fixes für Moltbot.", "Mexico City security guide for AI agents: LFPDPPP compliance, zero-trust and fast fixes for Moltbot.")
  return {
    title,
    description,
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4 text-gray-100">{pick(isDE, "AI Agent Security Mexico City", "AI Agent Security Mexico City")}</h1>
          <p className="text-lg text-gray-300 mb-4">
            {pick(isDE, "Mexico City Playbook für AI-Agent-Security: LFPDPPP, Zero-Trust und schnelle Fixes für Moltbot.", "Mexico City playbook for AI agent security: LFPDPPP, zero-trust and fast fixes for Moltbot.")}
          </p>
        </div>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{pick(isDE, "Mexico City Fokus", "Mexico City focus")}</h2>
          <div className="space-y-4">
            {[
              [pick(isDE, "LFPDPPP Compliance", "LFPDPPP compliance"), pick(isDE, "Datenschutzanforderungen sauber abbilden und dokumentieren.", "Map and document privacy requirements cleanly.")],
              [pick(isDE, "Payment Exposure", "Payment exposure"), pick(isDE, "Payment- und Banking-APIs strikt absichern.", "Lock down payment and banking APIs.")],
              [pick(isDE, "Third-Party Risk", "Third-party risk"), pick(isDE, "Integrationen mit lokalen Providern nur per Allowlist.", "Allowlist-only integrations with local providers.")],
              [pick(isDE, "Incident Handling", "Incident handling"), pick(isDE, "IR-Playbooks für Cross-Border Incidents.", "IR playbooks for cross-border incidents.")],
            ].map(([t, d]) => (
        ...
        <section className="mb-10">
          ...
          <div className="space-y-6">
            {[
              ...
              [4, pick(isDE, "Audit Logs an SIEM", "Audit logs to SIEM"), pick(isDE, "LFPDPPP-konforme Nachverfolgbarkeit.", "LFPDPPP-compliant traceability.")],
            ].map(([n, t, d]) => (
