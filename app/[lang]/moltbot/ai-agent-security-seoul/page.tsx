...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = pick(isDE, "AI Agent Security Seoul: PIPA, Zero-Trust, Moltbot | ClawGuru", "AI Agent Security Seoul: PIPA, Zero-Trust, Moltbot | ClawGuru")
  const description = pick(isDE, "Seoul Security Guide für AI-Agents: PIPA-Compliance, Zero-Trust und schnelle Fixes für Moltbot.", "Seoul security guide for AI agents: PIPA compliance, zero-trust and fast fixes for Moltbot.")
  return {
    title,
    description,
    keywords: ["ai agent security seoul", "pipa compliance", "moltbot security", "zero trust ai"],
...
export default function AIAgentSecuritySeoulPage({ params }: PageProps) {
  const locale = (SUPPORTED_LOCALES.includes(params.lang as Locale) ? params.lang : "de") as Locale
  const isDE = locale === "de"
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4 text-gray-100">{pick(isDE, "AI Agent Security Seoul", "AI Agent Security Seoul")}</h1>
          <p className="text-lg text-gray-300 mb-4">
            {pick(isDE, "Seoul Playbook für AI-Agent-Security: PIPA, Zero-Trust und schnelle Fixes für Moltbot.", "Seoul playbook for AI agent security: PIPA, zero-trust and fast fixes for Moltbot.")}
          </p>
        </div>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{pick(isDE, "Seoul Fokus", "Seoul focus")}</h2>
          <div className="space-y-4">
            {[
              [pick(isDE, "PIPA Compliance", "PIPA compliance"), pick(isDE, "Datenzugriff, Logging und Retention nach PIPA ausrichten.", "Align data access, logging and retention with PIPA.")],
              [pick(isDE, "Telecom & SaaS Exposure", "Telecom & SaaS exposure"), pick(isDE, "Integrationen mit lokalen Providern hart absichern.", "Lock down integrations with local providers.")],
        ...
        <section className="mb-10">
          ...
          <div className="space-y-6">
            {[
              ...
              [4, pick(isDE, "Audit Logs an SIEM", "Audit logs to SIEM"), pick(isDE, "PIPA-konforme Nachverfolgbarkeit sichern.", "Ensure PIPA-compliant traceability.")],
            ].map(([n, t, d]) => (
