...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const title = pick(isDE, "AI Agent Tool Poisoning Prevention: KI-Agenten-Tool-Poisoning-Prävention | ClawGuru Moltbot", "AI Agent Tool Poisoning Prevention: AI Agent Tool Poisoning Prevention | ClawGuru Moltbot")
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-amber-900 border-l-4 border-amber-500 p-4 mb-8 text-sm text-amber-100">
          <strong className="text-amber-100">"Not a Pentest" Notice</strong>: {pick(isDE, "Tool-Poisoning-Prävention-Guide für eigene KI-Agenten-Systeme.", "Tool poisoning prevention guide for your own AI agent systems.")}
        </div>
        <div className="mb-3"><span className="text-xs font-bold uppercase tracking-widest text-cyan-400">Moltbot · Batch 30</span></div>
        <h1 className="text-4xl font-bold mb-4 text-gray-100">{pick(isDE, "AI Agent Tool Poisoning Prevention", "AI Agent Tool Poisoning Prevention")}</h1>
        <p className="text-lg text-gray-300 mb-6">
          {pick(isDE, "Tool Poisoning ist eine der gefährlichsten KI-Agenten-Angriffsklassen — ein kompromittiertes Tool kann den gesamten Agenten übernehmen. Vier Kontrollen: Tool Integrity Verification, Malicious Tool Detection, Tool Allowlisting und Output Validation.", "Tool poisoning is one of the most dangerous AI agent attack classes — a compromised tool can take over the entire agent. Four controls: tool integrity verification, malicious tool detection, tool allowlisting and output validation.")}
        </p>
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{pick(isDE, "4 Tool-Poisoning-Prävention-Kontrollen", "4 Tool Poisoning Prevention Controls")}</h2>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            <a href={`/${locale}/moltbot/agent-tool-security`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{pick(isDE, "Tool-Security-Overview", "Tool security overview")}</div>
            </a>
            ...
            <a href={`/${locale}/moltbot/ai-agent-escalation-prevention`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{pick(isDE, "Escalation-Prevention", "Escalation prevention")}</div>
            </a>
