...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const title = pick(isDE, "AI Agent Behavioral Monitoring: KI-Agenten-Verhaltensmonitoring | ClawGuru Moltbot", "AI Agent Behavioral Monitoring: AI Agent Behavioral Monitoring | ClawGuru Moltbot")
  const description = pick(isDE, "KI-Agenten-Verhaltensmonitoring: Behavior Baseline, Anomaly Detection, Behavior Profiling und Behavior Enforcement für KI-Agenten-Verhaltensüberwachung.", "AI agent behavioral monitoring: behavior baseline, anomaly detection, behavior profiling and behavior enforcement for AI agent behavior monitoring.")
  return {
    title, description,
    ...
    <div className="min-h-screen bg-[#0a0a0a] relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 relative z-10">
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify([
          ...jsonLd,
          { "@context": "https://schema.org", "@type": "WebPage", name: pick(isDE, "Moltbot AI Agent Behavioral Monitoring Guide", "Moltbot AI Agent Behavioral Monitoring Guide"), description: pick(isDE, "KI-Agenten-Verhaltensmonitoring", "AI agent behavioral monitoring"), url: `${SITE_URL}/${locale}${PATH}` }
        ]) }} />
        <div className="bg-amber-900/80 backdrop-blur-lg border-l-4 border-amber-500 p-4 mb-8 text-sm text-amber-100 rounded-r-lg shadow-lg animate-fade-in-up" style={{animationDelay: '0.2s'}}>
          <strong className="text-amber-100">"Not a Pentest" Trust-Anker</strong>: {pick(isDE, "Agent-Behavioral-Monitoring-Guide für eigene KI-Systeme.", "Agent behavioral monitoring guide for your own AI systems.")}
        </div>
        <div className="mb-8 animate-fade-in-up" style={{animationDelay: '0.3s'}}>
          <div className="mb-4"><span className="text-xs font-bold uppercase tracking-widest text-cyan-400 bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">Moltbot AI Security · Behavioral Monitoring</span></div>
          <h1 className="text-4xl font-bold mb-4 text-gray-100 bg-gradient-to-r from-gray-100 via-white to-gray-100 bg-clip-text text-transparent">
            {pick(isDE, "AI Agent Behavioral Monitoring", "AI Agent Behavioral Monitoring")}
          </h1>
          <p className="text-lg text-gray-300 mb-6 leading-relaxed">
            {pick(isDE, "KI-Agenten ohne Behavioral Monitoring können kompromittiert werden und missbräuchliche Aktivitäten ausführen — ohne Überwachung bleibt dies unbemerkt. Vier Kontrollen: Behavior Baseline, Anomaly Detection, Behavior Profiling und Behavior Enforcement.", "AI agents without behavioral monitoring can be compromised and perform malicious activities — without monitoring, this goes unnoticed. Four controls: behavior baseline, anomaly detection, behavior profiling and behavior enforcement.")}
          </p>
        </div>

        {/* Amateur Section */}
        <section className="mb-10 animate-fade-in-up" style={{animationDelay: '0.4s'}}>
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{pick(isDE, "Was ist Behavioral Monitoring für KI-Agenten? Einfach erklärt", "What is Behavioral Monitoring for AI Agents? Simply Explained")}</h2>
          <div className="bg-gray-800/80 backdrop-blur-lg p-6 rounded-xl border border-gray-700/50 shadow-2xl hover:border-cyan-500/30 transition-all duration-300 hover:shadow-cyan-500/20">
            <p className="text-gray-300 leading-relaxed mb-4">
              {pick(isDE, "Behavioral Monitoring für KI-Agenten ist wie ein Überwachungssystem für Mitarbeiter-Aktivitäten: es trackt, was ein Agent normalerweise tut (Tool-Calls, API-Aufrufe, Ressourcen-Nutzung) und meldet Abweichungen. Behavior Baseline legt fest, was 'normal' ist. Anomaly Detection erkennt ungewöhnliche Muster. Behavior Profiling analysiert Trends über Zeit. Behavior Enforcement blockiert Agenten, die Grenzen überschreiten. Wenn ein Agent durch Prompt Injection kompromittiert wird, zeigt Behavioral Monitoring sofort abnormales Verhalten — z.B. plötzlich viele API-Calls oder Zugriff auf unerwartete Endpunkte.", "Behavioral monitoring for AI agents is like an employee activity monitoring system: it tracks what an agent normally does (tool calls, API calls, resource usage) and flags deviations. Behavior baseline establishes what's 'normal'. Anomaly detection detects unusual patterns. Behavior profiling analyzes trends over time. Behavior enforcement blocks agents that exceed limits. If an agent is compromised via prompt injection, behavioral monitoring immediately shows abnormal behavior — e.g., sudden spikes in API calls or access to unexpected endpoints.")}
            </p>
            <p className="text-gray-400 text-sm">↓ {pick(isDE, "Springe zu 4 Kontrollen und FAQ", "Jump to 4 controls and FAQ")}</p>
          </div>
        </section>

        <section className="mb-10 animate-fade-in-up" style={{animationDelay: '0.5s'}}>
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{pick(isDE, "4 Agent-Behavioral-Monitoring-Kontrollen", "4 Agent Behavioral Monitoring Controls")}</h2>
        ...
        <section className="mb-10 animate-fade-in-up" style={{animationDelay: '0.7s'}}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <a href={`/${locale}/moltbot/ai-agent-audit-logging`} className="block bg-gray-800/80 backdrop-blur-lg p-4 rounded-xl border border-gray-700/50 hover:border-cyan-500/30 transition-all duration-300 shadow-xl">
              ...
              <div className="text-sm text-gray-300">{pick(isDE, "Behavior-Logging", "Behavior logging")}</div>
            </a>
            <a href={`/${locale}/moltbot/ai-agent-rbac`} className="block bg-gray-800/80 backdrop-blur-lg p-4 rounded-xl border border-gray-700/50 hover:border-cyan-500/30 transition-all duration-300 shadow-xl">
              ...
              <div className="text-sm text-gray-300">{pick(isDE, "Behavior-Permissions", "Behavior permissions")}</div>
            </a>
            <a href={`/${locale}/moltbot/ai-incident-response`} className="block bg-gray-800/80 backdrop-blur-lg p-4 rounded-xl border border-gray-700/50 hover:border-cyan-500/30 transition-all duration-300 shadow-xl">
              ...
              <div className="text-sm text-gray-300">{pick(isDE, "Behavior-Incident-Response", "Behavior incident response")}</div>
            </a>
            <a href={`/${locale}/moltbot/ai-agent-security`} className="block bg-gray-800/80 backdrop-blur-lg p-4 rounded-xl border border-gray-700/50 hover:border-cyan-500/30 transition-all duration-300 shadow-xl">
              ...
              <div className="text-sm text-gray-300">{pick(isDE, "Monitoring-Overview", "Monitoring overview")}</div>
            </a>
          </div>
        </section>
          ...
          <div className="bg-gradient-to-r from-cyan-900/80 to-blue-900/80 backdrop-blur-lg p-6 rounded-xl border border-cyan-700/50 shadow-2xl hover:border-cyan-500/30 transition-all duration-300 hover:shadow-cyan-500/20">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-cyan-200 mb-3">Security Research &amp; Engineering · Behavioral Monitoring Specialists</div>
                <div className="flex items-center gap-4 text-xs text-cyan-300 mb-3">
                  <span>📅 {pick(isDE, 'Veröffentlicht', 'Published')}: 28.04.2026</span>
                  <span>🔄 {pick(isDE, 'Zuletzt geprüft', 'Last reviewed')}: 28.04.2026</span>
                </div>
                <div className="text-sm text-cyan-100 leading-relaxed">
                  {pick(isDE, 'Dieser Guide basiert auf praktischer Erfahrung mit Behavioral-Monitoring-Implementierungen für KI-Agenten in Produktionsumgebungen. Die beschriebenen Best Practices sind in echten Deployments erprobt und kontinuierlich verbessert worden.', 'This guide is based on practical experience with behavioral monitoring implementations for AI agents in production environments. The described best practices have been proven in real deployments and continuously improved.')}
                </div>
              </div>
            </div>
