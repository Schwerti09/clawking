...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = pick(isDE, "AI Agent Security Sao Paulo: LGPD, Zero-Trust, Moltbot | ClawGuru", "AI Agent Security Sao Paulo: LGPD, Zero-Trust, Moltbot | ClawGuru")
  const description = pick(isDE, "Sao Paulo Security Guide für AI-Agents: LGPD-Compliance, Zero-Trust und schnelle Fixes für Moltbot.", "Sao Paulo security guide for AI agents: LGPD compliance, zero-trust and fast fixes for Moltbot.")
  return {
    title,
    description,
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4 text-gray-100">{pick(isDE, "AI Agent Security Sao Paulo", "AI Agent Security Sao Paulo")}</h1>
          <p className="text-lg text-gray-300 mb-4">
            {pick(isDE, "Sao Paulo Playbook für AI-Agent-Security: LGPD, Zero-Trust und schnelle Fixes für Moltbot.", "Sao Paulo playbook for AI agent security: LGPD, zero-trust and fast fixes for Moltbot.")}
          </p>
        </div>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{pick(isDE, "Sao Paulo Fokus", "Sao Paulo focus")}</h2>
          <div className="space-y-4">
            {[
              [pick(isDE, "LGPD Compliance", "LGPD compliance"), pick(isDE, "Datenschutzanforderungen sauber abbilden und dokumentieren.", "Map and document privacy requirements cleanly.")],
              [pick(isDE, "FinTech Exposure", "FinTech exposure"), pick(isDE, "Payment- und Banking-Integrationen hart absichern.", "Lock down payment and banking integrations.")],
              [pick(isDE, "Third-Party Risk", "Third-party risk"), pick(isDE, "Lokale Provider nur per Allowlist integrieren.", "Integrate local providers via allowlist only.")],
        ...
        <section className="mb-10">
          ...
          <div className="space-y-6">
            {[
              ...
              [4, pick(isDE, "Audit Logs an SIEM", "Audit logs to SIEM"), pick(isDE, "LGPD-konforme Nachverfolgbarkeit.", "LGPD-compliant traceability.")],
            ].map(([n, t, d]) => (
