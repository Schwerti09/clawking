...
const PATH = "/moltbot/ai-agent-security-sao-paulo"
...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const title = pick(isDE, "AI Agent Security Sao Paulo: LGPD, Zero-Trust, Moltbot 2026 | ClawGuru", "AI Agent Security Sao Paulo: LGPD, Zero-Trust, Moltbot 2026 | ClawGuru")
  const description = pick(isDE, "Sao Paulo Security Guide für AI-Agents: LGPD-Compliance (Lei Geral de Proteção de Dados), Zero-Trust, Banking Security und sofortige Fixes für Moltbot-Deployments.", "Sao Paulo security guide for AI agents: LGPD compliance (General Data Protection Law), zero-trust, banking security and immediate fixes for Moltbot deployments.")
  return {
    title, description,
    keywords: ["ai agent security sao paulo", "lgpd compliance brazil", "moltbot security brazil", "zero trust ai", "banking security"],
...
export default function AIAgentSecuritySaoPauloPage({ params }: { params: { lang: string } }) {
  const locale = (SUPPORTED_LOCALES.includes(params.lang as Locale) ? params.lang : "de") as Locale
  ...
  const FAQ = [
    { q: pick(isDE, "Was ist LGPD in Brasilien?", "What is LGPD in Brazil?"), a: pick(isDE, "LGPD (Lei Geral de Proteção de Dados) ist Brasiliens Datenschutzgesetz. Es regelt Consent, Data Access, Breach Notification und Data Subject Rights. Verstöße können zu hohen Strafen führen.", "LGPD (General Data Protection Law) is Brazil's data protection law. It regulates consent, data access, breach notification and data subject rights. Violations can lead to heavy fines.") },
    { q: pick(isDE, "Warum ist Banking Security in São Paulo wichtig?", "Why is banking security important in São Paulo?"), a: pick(isDE, "São Paulo ist Brasiliens Banking-Hub. Payment- und Banking-APIs müssen strikt abgesichert werden: PCI DSS Compliance, mTLS, API-Key Rotation, Audit Logs.", "São Paulo is Brazil's banking hub. Payment and banking APIs must be strictly secured: PCI DSS compliance, mTLS, API key rotation, audit logs.") },
    { q: pick(isDE, "Welche Compliance-Regeln gelten in Brasilien?", "Which compliance rules apply in Brazil?"), a: pick(isDE, "LGPD (Lei Geral de Proteção de Dados), PCI DSS, FinTech Regulations. Alle erfordern: Consent, Data Residency, Breach Notification, Audit Trails.", "LGPD (General Data Protection Law), PCI DSS, FinTech regulations. All require: consent, data residency, breach notification, audit trails.") },
  ...
  const jsonLd = [
    { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "AI Agent Security Sao Paulo", item: `${SITE_URL}/${locale}${PATH}` },
    ]},
    { "@context": "https://schema.org", "@type": "Person", name: "R. Schwertfechter", jobTitle: "Principal Ops-Engineer & Security Architect", knowsAbout: ["AI Agent Security", "LGPD Compliance", "Brazil Security", "Zero-Trust", "Banking Security"] },
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 relative z-10 flex gap-8">
          ...
          <div className="sticky top-4">
            <div className="bg-gray-800/80 backdrop-blur-lg p-4 rounded-xl border border-gray-700/50 shadow-2xl">
              ...
              <nav className="space-y-2 text-sm">
                <a href="#amateur-section" className="block text-gray-300 hover:text-cyan-400 transition-colors">{pick(isDE, "Was ist Sao Paulo Security?", "What is Sao Paulo Security?")}</a>
                <a href="#deep-dive" className="block text-gray-300 hover:text-cyan-400 transition-colors">{pick(isDE, "Sao Paulo Fokus", "Sao Paulo focus")}</a>
                ...
                <a href="#calculator" className="block text-gray-300 hover:text-cyan-400 transition-colors">{pick(isDE, "Sao Paulo Security Score", "Sao Paulo Security Score")}</a>
              </nav>
        ...
        <div className="flex-1">
          ...
          <div className="mb-8 animate-fade-in-up">
            <div className="mb-4">
              <span className="text-xs font-bold uppercase tracking-widest text-cyan-400 bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">Sao Paulo Security · LGPD-First</span>
            </div>
            <h1 className="text-4xl font-bold mb-4 text-gray-100 bg-gradient-to-r from-gray-100 via-white to-gray-100 bg-clip-text text-transparent">
              {pick(isDE, "AI Agent Security Sao Paulo — Du deployst in Brasilien ohne LGPD, ohne Zero-Trust, ohne Banking Security. Compliance-Verstoß, Daten-Leak, dein CEO hat den CISO gefeuert.", "AI Agent Security Sao Paulo — You deploy in Brazil without LGPD, without zero-trust, without banking security. Compliance violation, data leak, your CEO fired the CISO.")}
            </h1>
            <p className="text-lg text-gray-300 mb-6 leading-relaxed">
              {pick(isDE, "Du deployst in Brasilien ohne LGPD, ohne Zero-Trust, ohne Banking Security. Compliance-Verstoß, Daten-Leak, dein CEO hat den CISO gefeuert. Hier ist, wie du das verhinderst.", "You deploy in Brazil without LGPD, without zero-trust, without banking security. Compliance violation, data leak, your CEO fired the CISO. Here's how to prevent it.")}
            </p>
          </div>
          ...
          <section id="amateur-section" className="mb-10 animate-fade-in-up" style={{animationDelay: '0.1s'}}>
            <div className="bg-gray-800/80 backdrop-blur-lg p-6 rounded-xl border border-gray-700/50 shadow-2xl">
              <h2 className="text-2xl font-semibold text-cyan-400 mb-4">{pick(isDE, "Was ist Sao Paulo Security? Einfach erklärt.", "What is Sao Paulo Security? Simply explained.")}</h2>
              <p className="text-gray-300 leading-relaxed mb-4">
                {pick(isDE, "Sao Paulo Security bedeutet LGPD-First Deployment in Brasilien: LGPD (Lei Geral de Proteção de Dados), Zero-Trust (keine implizite Vertrauenswürdigkeit), Banking Security (PCI DSS, Banking-Integrationen). Gute Sao Paulo Security bedeutet: LGPD einhalten, Banking-Integrationen sichern, schnell auf Incidents reagieren.", "Sao Paulo security means LGPD-first deployment in Brazil: LGPD (General Data Protection Law), zero-trust (no implicit trust), banking security (PCI DSS, banking integrations). Good Sao Paulo security means: comply with LGPD, secure banking integrations, respond fast to incidents.")}
              </p>
          ...
          <section id="deep-dive" className="mb-10 animate-fade-in-up" style={{animationDelay: '0.2s'}}>
            <h2 className="text-3xl font-bold text-gray-100 mb-6">{pick(isDE, "Sao Paulo Fokus", "Sao Paulo focus")}</h2>
            <div className="space-y-4">
              {[
                { t: pick(isDE, "LGPD Compliance", "LGPD compliance"), d: pick(isDE, "Datenschutzanforderungen sauber abbilden und dokumentieren. Verstöße können zu hohen Strafen führen.", "Map and document privacy requirements cleanly. Violations can lead to heavy fines.") },
                { t: pick(isDE, "FinTech Exposure", "FinTech exposure"), d: pick(isDE, "Payment- und Banking-Integrationen hart absichern.", "Lock down payment and banking integrations.") },
                { t: pick(isDE, "Third-Party Risk", "Third-party risk"), d: pick(isDE, "Lokale Provider nur per Allowlist integrieren.", "Integrate local providers via allowlist only.") },
          ...
          <section id="scars" className="mb-10 animate-fade-in-up" style={{animationDelay: '0.3s'}}>
            ...
            <div className="bg-red-900/20 border-l-4 border-red-500 p-6 rounded-r-lg mb-4">
              ...
              <p className="text-gray-300 mb-3">{pick(isDE, "Keine LGPD Compliance, Daten ohne Consent verarbeitet. LGPD-Verstoß, Strafe 50M BRL. Fix: Aktiviere LGPD-konforme Consent-Management.", "No LGPD compliance, data processed without consent. LGPD violation, fine 50M BRL. Fix: Enable LGPD-compliant consent management.")}</p>
          ...
          <section id="controls" className="mb-10 animate-fade-in-up" style={{animationDelay: '0.4s'}}>
            ...
            <div className="space-y-4">
              {[
                ...
                { n: 4, t: pick(isDE, "Audit Logs an SIEM", "Audit logs to SIEM"), d: pick(isDE, "LGPD-konforme Nachverfolgbarkeit.", "LGPD-compliant traceability.") },
              ].map((item) => (
          ...
          <section id="checklist" className="mb-10 animate-fade-in-up" style={{animationDelay: '0.5s'}}>
            <h2 className="text-3xl font-bold text-gray-100 mb-6">{pick(isDE, "Interaktive Sao Paulo Security Checkliste", "Interactive Sao Paulo Security Checklist")}</h2>
            <div className="bg-gray-800/80 backdrop-blur-lg p-6 rounded-xl border border-gray-700/50 shadow-2xl">
              <div className="space-y-3">
                {[
                  { id: "sp1", text: pick(isDE, "LGPD Consent Management aktiviert", "LGPD consent management enabled") },
                  { id: "sp2", text: pick(isDE, "PCI DSS Compliance aktiviert", "PCI DSS compliance enabled") },
                  { id: "sp3", text: pick(isDE, "Banking-Integration Allowlist konfiguriert", "Banking integration allowlist configured") },
                  { id: "sp4", text: pick(isDE, "mTLS für Banking-Traffic aktiviert", "mTLS for banking traffic enabled") },
                  { id: "sp5", text: pick(isDE, "API-Key Rotation aktiviert", "API key rotation enabled") },
                  { id: "sp6", text: pick(isDE, "Audit Logs an SIEM", "Audit logs to SIEM") },
                  { id: "sp7", text: pick(isDE, "LGPD Breach Notification Playbook", "LGPD breach notification playbook") },
                  { id: "sp8", text: pick(isDE, "Banking Security Audit durchgeführt", "Banking security audit done") },
                ].map((item) => (
          ...
          {/* Sao Paulo Security Score Calculator */}
          <section id="calculator" className="mb-10 animate-fade-in-up" style={{animationDelay: '0.6s'}}>
            <h2 className="text-3xl font-bold text-gray-100 mb-6">{pick(isDE, "Sao Paulo Security Score Calculator", "Sao Paulo Security Score Calculator")}</h2>
            <div className="bg-gray-800/80 backdrop-blur-lg p-6 rounded-xl border border-gray-700/50 shadow-2xl">
              <div className="space-y-4">
                {[
              ...
              <div className="mt-6 pt-6 border-t border-gray-700">
                <div className="flex items-center justify-between">
                  <span className="text-gray-300">{pick(isDE, "Dein Sao Paulo Security Score:", "Your Sao Paulo Security Score:")}</span>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm text-cyan-100 leading-relaxed mb-4">
                    {pick(isDE, "15+ Jahre Erfahrung als Ops-Engineer, Incident Responder und Security Architect. Experte für LGPD Compliance, Brazil Security, Zero-Trust und Banking Security.", "15+ years experience as Ops-Engineer, Incident Responder and Security Architect. Expert in LGPD compliance, Brazil security, zero-trust and banking security.")}
                  </div>
                </div>
              </div>
            </div>
