...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const title = isDE
    ? "LLM Quantization Security: LLM-Quantization-Security | ClawGuru Moltbot"
    : "LLM Quantization Security: LLM Quantization Security | ClawGuru Moltbot"
  const description = isDE
    ? "LLM-Quantization-Security: Quantization Attack Detection, Quantization-Aware Training, Secure Quantization und Quantization Integrity Verification für LLM-Quantization-Security."
    : "LLM quantization security: quantization attack detection, quantization-aware training, secure quantization and quantization integrity verification for LLM quantization security."
  return {
    title, description,
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-amber-900 border-l-4 border-amber-500 p-4 mb-8 text-sm text-amber-100">
          <strong className="text-amber-100">"Not a Pentest" Notice</strong>: {isDE ? "Quantization-Security-Guide für eigene KI-Systeme." : "Quantization security guide for your own AI systems."}
        </div>
        <div className="mb-3"><span className="text-xs font-bold uppercase tracking-widest text-cyan-400">Moltbot · Batch 23</span></div>
        <h1 className="text-4xl font-bold mb-4 text-gray-100">{isDE ? "LLM Quantization Security" : "LLM Quantization Security"}</h1>
        <p className="text-lg text-gray-300 mb-6">
          {isDE
            ? "LLM-Modelle ohne Quantization-Security sind anfällig für Quantization-Attacken — ohne Quantization-Security bleibt Quantization ungeschützt. Vier Kontrollen: Quantization Attack Detection, Quantization-Aware Training, Secure Quantization und Quantization Integrity Verification."
            : "LLM models without quantization security are vulnerable to quantization attacks — without quantization security, quantization remains unprotected. Four controls: quantization attack detection, quantization-aware training, secure quantization and quantization integrity verification."}
        </p>
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{isDE ? "4 Quantization-Security-Kontrollen" : "4 Quantization Security Controls"}</h2>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <a href={`/${locale}/moltbot/ai-agent-security`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "Quantization-Overview" : "Quantization overview"}</div>
            </a>
          </div>
        </section>
      </div>
