  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4 text-gray-100">{pick(isDE, "AI Agent Secrets Management", "AI Agent Secrets Management")}</h1>
        ...
        <section className="mb-10">
          ...
          <div className="space-y-4">
            {[
              ["1. Zero Hardcoded Secrets", pick(isDE, "Keine Secrets im Code, in Config-Dateien oder in Environment-Variablen direkt. Ausnahmslos externalisierte Secret-Verwaltung.", "No secrets in code, config files or environment variables directly. Exclusively externalized secret management.")],
              ["2. Dynamic Secrets", pick(isDE, "Kurzlebige, dynamisch generierte Credentials für jeden AI-Agent-Aufruf. HashiCorp Vault Dynamic Secrets für Datenbanken und APIs.", "Short-lived, dynamically generated credentials for every AI agent call. HashiCorp Vault dynamic secrets for databases and APIs.")],
              ["3. Automatic Key Rotation", pick(isDE, "Automatische Rotation aller API Keys und Credentials. Kein manueller Prozess, der vergessen werden kann.", "Automatic rotation of all API keys and credentials. No manual process that can be forgotten.")],
              ["4. Least Privilege Access", pick(isDE, "Jeder AI-Agent erhält nur die minimal notwendigen Credentials. Service-Account pro Agent-Typ mit spezifischen Berechtigungen.", "Each AI agent receives only the minimally necessary credentials. Service account per agent type with specific permissions.")],
              ["5. Secret Scanning", pick(isDE, "Automatisches Scannen von Code und Commits auf versehentlich eingecheckte Secrets. GitGuardian, Trufflehog oder GitHub Secret Scanning.", "Automatic scanning of code and commits for accidentally checked-in secrets. GitGuardian, Trufflehog or GitHub Secret Scanning.")],
            ].map(([t, d]) => (
        ...
        <section className="mb-10">
          ...
          <div className="space-y-6">
            {[
              [1, pick(isDE, "Secret Inventory erstellen", "Create secret inventory"), pick(isDE, "Alle verwendeten Secrets und API Keys inventarisieren. Wer hat Zugriff? Wann wurde zuletzt rotiert?", "Inventory all used secrets and API keys. Who has access? When was last rotation?")],
              [2, pick(isDE, "Vault oder AWS Secrets Manager einrichten", "Set up Vault or AWS Secrets Manager"), pick(isDE, "Central Secret Store deployen. HashiCorp Vault On-Premise oder managed Service.", "Deploy central secret store. HashiCorp Vault on-premise or managed service.")],
              [3, pick(isDE, "Hardcoded Secrets entfernen", "Remove hardcoded secrets"), pick(isDE, "Code auf hardcoded Secrets scannen. Trufflehog oder GitGuardian für historische Commits.", "Scan code for hardcoded secrets. Trufflehog or GitGuardian for historical commits.")],
              [4, pick(isDE, "Dynamic Secrets konfigurieren", "Configure dynamic secrets"), pick(isDE, "Vault Dynamic Secrets für alle Datenbankverbindungen. TTL von maximal 1 Stunde für AI-Agent-Credentials.", "Vault dynamic secrets for all database connections. TTL of maximum 1 hour for AI agent credentials.")],
              [5, pick(isDE, "Rotation automatisieren", "Automate rotation"), pick(isDE, "Automatische Rotation aller statischen Secrets einrichten. Alert wenn Rotation fehlschlägt.", "Set up automatic rotation of all static secrets. Alert when rotation fails.")],
            ].map(([n, t, d]) => (
