...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const title = isDE
    ? "AI Agent RBAC: Role-Based Access Control für KI-Agenten | ClawGuru"
    : "AI Agent RBAC: Role-Based Access Control for AI Agents | ClawGuru"
  const description = isDE
    ? "RBAC-Design für KI-Agenten: Rollen, Berechtigungen und Tool-Zugriff nach Least-Privilege-Prinzip. Moltbot RBAC-Konfiguration, Dynamic Permission Scoping und Agent Identity Management."
    : "RBAC design for AI agents: roles, permissions and tool access following least-privilege principles. Moltbot RBAC configuration, dynamic permission scoping and agent identity management."
  return {
    title, description,
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-amber-900 border-l-4 border-amber-500 p-4 mb-8 text-sm text-amber-100">
          <strong className="text-amber-100">"Not a Pentest" Notice</strong>: {isDE ? "RBAC-Leitfaden für eigene KI-Agent-Systeme." : "RBAC guide for your own AI agent systems."}
        </div>
        ...
        <h1 className="text-4xl font-bold mb-4 text-gray-100">
          {isDE ? "AI Agent RBAC: Role-Based Access Control" : "AI Agent RBAC: Role-Based Access Control"}
        </h1>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{isDE ? "4 Standard-Agenten-Rollen" : "4 Standard Agent Roles"}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {RBAC_ROLES.map((r) => (
              <div key={r.role} className={`${colorMap[r.color]} border rounded-lg p-4`}>
                ...
                <div className="text-xs text-green-400 mb-1">✓ {isDE ? "Erlaubt:" : "Allowed:"}</div>
                <ul className="text-xs text-green-300 mb-2">{r.tools.map((t) => <li key={t}>▸ {t}</li>)}</ul>
                <div className="text-xs text-red-400 mb-1">✗ {isDE ? "Blockiert:" : "Blocked:"}</div>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{isDE ? "Moltbot RBAC Konfiguration" : "Moltbot RBAC Configuration"}</h2>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            <a href={`/${locale}/moltbot/zero-trust-ai-agents`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "Capability Tokens" : "Capability tokens"}</div>
            </a>
            <a href={`/${locale}/moltbot/secure-agent-deployment`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "K8s ServiceAccount Identity" : "K8s ServiceAccount identity"}</div>
            </a>
            <a href={`/${locale}/moltbot/ai-incident-response`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "Wenn RBAC umgangen wird" : "When RBAC gets bypassed"}</div>
            </a>
            <a href={`/${locale}/solutions/zero-trust-architecture`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "RBAC im ZT-Kontext" : "RBAC in ZT context"}</div>
            </a>
          </div>
        </section>
      </div>
