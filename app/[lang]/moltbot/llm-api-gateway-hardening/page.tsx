...
export default function LlmApiGatewayHardeningPage({ params }: { params: { lang: string } }) {
  ...
  const jsonLd = [
    ...
    { "@context": "https://schema.org", "@type": "WebPage", name: pick(isDE, "LLM API Gateway Hardening Guide", "LLM API Gateway Hardening Guide"), description: pick(isDE, "LLM API Gateway Sicherheit", "LLM API gateway security"), url: `${SITE_URL}/${locale}${PATH}` },
  ]
    ...
    <div className="min-h-screen bg-[#0a0a0a] relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 relative z-10">
        ...
        <div className="bg-amber-900/80 backdrop-blur-lg border-l-4 border-amber-500 p-4 mb-8 text-sm text-amber-100 rounded-r-lg shadow-lg animate-fade-in-up" style={{animationDelay: '0.2s'}}>
          <strong className="text-amber-100">"Not a Pentest" Trust-Anker</strong>: {pick(isDE, "API-Gateway-Hardening-Guide für eigene KI-Infrastruktur.", "API gateway hardening guide for your own AI infrastructure.")}
        </div>
        <div className="mb-8 animate-fade-in-up" style={{animationDelay: '0.3s'}}>
          <div className="mb-4"><span className="text-xs font-bold uppercase tracking-widest text-cyan-400 bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">Moltbot AI Security · LLM API Gateway Hardening</span></div>
          <h1 className="text-4xl font-bold mb-4 text-gray-100 bg-gradient-to-r from-gray-100 via-white to-gray-100 bg-clip-text text-transparent">{pick(isDE, "LLM API Gateway Hardening", "LLM API Gateway Hardening")}</h1>
          <p className="text-lg text-gray-300 mb-6 leading-relaxed">{pick(isDE, "Direkter Zugriff auf LLM-APIs aus Anwendungen ist unsicher: kein Rate-Limiting, keine Audit-Logs, kein Key-Management. Ein zentrales Gateway löst alle diese Probleme auf einmal.", "Direct LLM API access from applications is insecure: no rate limiting, no audit logs, no key management. A central gateway solves all these problems at once.")}</p>
        </div>

        {/* Amateur Section */}
        <section className="mb-10 animate-fade-in-up" style={{animationDelay: '0.4s'}}>
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{pick(isDE, "Was ist ein LLM API Gateway? Einfach erklärt", "What is an LLM API Gateway? Simply Explained")}</h2>
          <div className="bg-gray-800/80 backdrop-blur-lg p-6 rounded-xl border border-gray-700/50 shadow-2xl hover:border-cyan-500/30 transition-all duration-300 hover:shadow-cyan-500/20">
            <p className="text-gray-300 leading-relaxed mb-4">
              {pick(isDE, "Ein LLM API Gateway ist wie ein Sicherheits-Tor für KI-Zugriffe: Anwendungen rufen nicht direkt OpenAI/Anthropic auf, sondern das Gateway. Das Gateway verwaltet API-Keys, setzt Rate-Limits, protokolliert alle Requests, und filtert gefährliche Inputs. Ohne Gateway kann eine kompromittierte App alle API-Keys exfiltrieren. Mit Gateway hat jede App nur ein JWT-Token — das Gateway hält die echten API-Keys sicher im Vault.", "An LLM API gateway is like a security gate for AI access: applications don't call OpenAI/Anthropic directly, but the gateway. The gateway manages API keys, sets rate limits, logs all requests, and filters dangerous inputs. Without a gateway, a compromised app can exfiltrate all API keys. With a gateway, each app only has a JWT token — the gateway holds the actual API keys securely in Vault.")}
            </p>
            <p className="text-gray-400 text-sm">↓ {pick(isDE, "Springe zu Gateway-Controls", "Jump to gateway controls")}</p>
          </div>
        </section>
        ...
        <section className="mb-10 animate-fade-in-up" style={{animationDelay: '0.7s'}}>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <a href={`/${locale}/moltbot/llm-rate-limiting`} className="block bg-gray-800/80 backdrop-blur-lg p-4 rounded-xl border border-gray-700/50 hover:border-cyan-500/30 transition-all duration-300 shadow-xl">
            ...
            <a href={`/${locale}/openclaw/network-segmentation-guide`} className="block bg-gray-800/80 backdrop-blur-lg p-4 rounded-xl border border-gray-700/50 hover:border-cyan-500/30 transition-all duration-300 shadow-xl">
              <div className="font-semibold text-cyan-400">{pick(isDE, "Network Segmentation", "Network Segmentation")}</div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-cyan-100 leading-relaxed">
                  {pick(isDE, 'Dieser Guide basiert auf praktischer Erfahrung mit LLM API Gateway Hardening-Implementierungen für KI-Systeme in Produktionsumgebungen. Die beschriebenen Best Practices sind in echten Deployments erprobt und kontinuierlich verbessert worden.', 'This guide is based on practical experience with LLM API gateway hardening implementations for AI systems in production environments. The described best practices have been proven in real deployments and continuously improved.')}
                </div>
              </div>
            </div>
