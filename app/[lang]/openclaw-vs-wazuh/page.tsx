    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-amber-900 border-l-4 border-amber-500 p-4 mb-8 text-sm text-amber-100">
          ...
          {pick(isDE, ': Dieser Guide hilft dir, das richtige Self-Hosted-Security-Tool zu wählen. Kein Angriffs-Tool.', ': This guide helps you choose the right self-hosted security tool. No attack tools.')}
        </div>

        <h1 className="text-4xl font-bold mb-4 text-gray-100">
          {pick(isDE, 'OpenClaw vs Wazuh: Self-Hosted Security Platform Vergleich', 'OpenClaw vs Wazuh: Self-Hosted Security Platform Comparison')}
        </h1>
        <p className="text-lg text-gray-300 mb-8">
          {pick(isDE, 'Wazuh ist ein Open-Source SIEM/XDR für Log-Analyse, Intrusion Detection und File Integrity Monitoring. OpenClaw (Teil der ClawGuru-Plattform) ergänzt mit Executable Runbooks, automatisierten Security-Checks und einem integrierten Compliance-Dashboard — optimiert für Self-Hoster.', 'Wazuh is an open-source SIEM/XDR for log analysis, intrusion detection and file integrity monitoring. OpenClaw (part of the ClawGuru platform) adds executable runbooks, automated security checks and an integrated compliance dashboard — optimised for self-hosters.')}
        </p>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-green-900 border border-green-700 p-5 rounded-lg">
              <h3 className="font-bold text-green-300 mb-3">🔓 {pick(isDE, 'OpenClaw ist die Wahl wenn...', 'OpenClaw is the choice when...')}</h3>
            ...
            <div className="bg-blue-900 border border-blue-700 p-5 rounded-lg">
              <h3 className="font-bold text-blue-300 mb-3">🦅 {pick(isDE, 'Wazuh ist die Wahl wenn...', 'Wazuh is the choice when...')}</h3>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            <a href={`/${lang}/openclaw/intrusion-detection-setup`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              <div className="font-semibold text-cyan-400">{pick(isDE, '🔍 IDS Setup', '🔍 IDS Setup')}</div>
              <div className="text-sm text-gray-300">{pick(isDE, 'Intrusion Detection Runbook', 'Intrusion Detection Runbook')}</div>
            </a>
            <a href={`/${lang}/openclaw/audit-logging-setup`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              <div className="font-semibold text-cyan-400">{pick(isDE, '📋 Audit Logging', '📋 Audit Logging')}</div>
              <div className="text-sm text-gray-300">{pick(isDE, 'Logging & Audit Runbook', 'Logging & Audit Runbook')}</div>
            </a>
          </div>
        </section>
      </div>
