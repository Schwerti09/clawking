...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = pick(isDE, "Roast Trends: Was ist heiß | ClawGuru", "Roast Trends: What's Hot | ClawGuru")
  const description = pick(isDE, "Trending Stacks, heiße Vulnerabilities und die schnellsten Score-Sprünge.", "Trending stacks, hot vulnerabilities, and fastest score jumps.")
  return {
    title,
    description,
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-4xl font-bold text-gray-100">{pick(isDE, "Roast Trends", "Roast Trends")}</h1>
          ...
          <p className="text-lg text-gray-300">
            {pick(isDE, "Was gerade heiß läuft: Trending Stacks, neue Vulnerabilities, Score-Sprünge.", "What's hot right now: trending stacks, new vulnerabilities, score jumps.")}
          </p>
        </div>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100 flex items-center gap-2">
            ...
            {pick(isDE, "Heiße Vulnerabilities", "Hot Vulnerabilities")}
          </h2>
          <div className="bg-gray-800 p-6 rounded-lg border border-gray-700 text-center">
            <p className="text-zinc-400 text-sm">{pick(isDE, "CVE-Tracking in Kürze verfügbar", "CVE tracking coming soon")}</p>
          </div>
        </section>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {trendingStacks.slice(0, 3).map((stack: any, idx: number) => (
                <div key={stack.name} className="bg-gradient-to-br from-green-900/30 to-gray-800 rounded-xl border border-green-700/50 p-4">
                  ...
                  <div className="text-xs text-green-300">{pick(isDE, "Elite Score", "Elite Score")}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-gradient-to-r from-amber-900/40 to-red-900/40 border border-amber-700/50 rounded-xl p-6 text-center">
          <h3 className="text-xl font-bold text-amber-300 mb-2">
            {pick(isDE, "Mach mit bei den Trends", "Join the trends")}
          </h3>
          <p className="text-sm text-amber-200/70 mb-4">
            {pick(isDE, "Roast deinen Stack und sieh, wo du im Ranking stehst", "Roast your stack and see where you rank")}
          </p>
          <Link 
