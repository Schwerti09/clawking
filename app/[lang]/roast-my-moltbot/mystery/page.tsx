...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = pick(isDE, "Mystery Stack | ClawGuru", "Mystery Stack | ClawGuru")
  const description = pick(isDE, "Rate den Stack anhand des Roasts — Täglich neues Rätsel", "Guess the stack from the roast — Daily puzzle")
  return {
    title,
    description,
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4 text-gray-100">
            {pick(isDE, "Mystery Stack", "Mystery Stack")}
          </h1>
          <p className="text-lg text-gray-300">
            {pick(isDE, "Rate den Tech-Stack anhand der Roast-Ergebnisse", "Guess the tech stack from the roast results")}
          </p>
        </div>
        ...
        {!mystery ? (
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-12 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-100 mb-2">{pick(isDE, "Kein Mystery verfügbar", "No mystery available")}</h2>
            <p className="text-zinc-400">
              {pick(isDE, "Noch keine Roast-Daten für Mystery verfügbar", "No roast data available for mystery yet")}
            </p>
          </div>
        ) : (
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 mb-8">
            ...
            <div className="flex gap-4 mb-6 text-sm text-zinc-500">
              <div className="flex items-center gap-1">
                ...
                <span>{pick(isDE, "Live", "Live")}</span>
              </div>
              <div className="flex items-center gap-1">
                <Users className="w-4 h-4" />
                <span>{stats?.totalRoasts || 0} {pick(isDE, "Roasts", "roasts")}</span>
              </div>
              <div className="flex items-center gap-1">
              ...
              <div className="space-y-3">
                {hints.length > 0 ? hints.map((hint: string, index: number) => (
                  ...
                  <div className="text-center text-zinc-500 py-4">
                    {pick(isDE, "Keine Hinweise verfügbar", "No hints available")}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="mb-4">
              <label className="text-sm text-zinc-500 block mb-2">
                {pick(isDE, "Deine Antwort", "Your answer")}
              </label>
              <input
                type="text"
                placeholder={pick(isDE, "z.B. Kubernetes + Docker + Postgres", "e.g. Kubernetes + Docker + Postgres")}
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-semibold text-gray-100">
              {pick(isDE, "Top Scores", "Top Scores")}
            </h3>
          </div>
        ...
        <div className="bg-amber-900 border-l-4 border-amber-500 p-4 mt-8 text-sm text-amber-100">
          <strong className="text-amber-100">"Not a Pentest" Notice</strong>: {pick(isDE, "Dies ist ein Rätselspiel zum Lernen. Keine echten Angriffe.", "This is a puzzle game for learning. No real attacks.")}
        </div>
      </div>
    </div>
  )
