...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = pick(isDE, "Team Roast Challenge | ClawGuru", "Team Roast Challenge | ClawGuru")
  const description = pick(isDE, "Roste als Team. Challenge deine Kollegen. Wer hat den sichersten Stack?", "Roast as a team. Challenge your colleagues. Who has the most secure stack?")
  return {
    title,
    description,
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-bold text-gray-100 mb-2">{pick(isDE, "Team Roast", "Team Roast")}</h1>
          <p className="text-lg text-gray-300">
            {pick(isDE, "Gemeinsam rosten, gemeinsam wachsen. Challenge dein Team!", "Roast together, grow together. Challenge your team!")}
          </p>
        </div>
        ...
        <section className="mb-10">
          <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-gray-800 rounded-xl border border-gray-700 p-4 text-center">
              ...
              <h3 className="font-semibold text-gray-100 mb-1">{pick(isDE, "1. Team bilden", "1. Form Team")}</h3>
              <p className="text-sm text-zinc-500">{pick(isDE, "Lade Kollegen ein, erstelle ein Team", "Invite colleagues, create a team")}</p>
            </div>
            <div className="bg-gray-800 rounded-xl border border-gray-700 p-4 text-center">
              <div className="w-12 h-12 bg-red-900/50 rounded-full flex items-center justify-center mx-auto mb-3">
                <Target className="w-6 h-6 text-red-400" />
              </div>
              <h3 className="font-semibold text-gray-100 mb-1">{pick(isDE, "2. Roaste", "2. Roast")}</h3>
              <p className="text-sm text-zinc-500">{pick(isDE, "Jeder rostet seinen Stack", "Everyone roasts their stack")}</p>
            </div>
            <div className="bg-gray-800 rounded-xl border border-gray-700 p-4 text-center">
              <div className="w-12 h-12 bg-green-900/50 rounded-full flex items-center justify-center mx-auto mb-3">
                <TrendingUp className="w-6 h-6 text-green-400" />
              </div>
              <h3 className="font-semibold text-gray-100 mb-1">{pick(isDE, "3. Gewinne", "3. Win")}</h3>
              <p className="text-sm text-zinc-500">{pick(isDE, "Sammle Punkte, steige auf", "Collect points, climb up")}</p>
            </div>
          </div>
        </section>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100 flex items-center gap-2">
            ...
            {pick(isDE, "Top Stacks", "Top Stacks")}
          </h2>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-zinc-500">{pick(isDE, "Anonymisiert", "Anonymized")}</div>
                      </div>
                    </div>
        ...
        <section className="mb-10">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-gradient-to-br from-amber-900/30 to-gray-800 rounded-xl border border-amber-700/50 p-4">
              <div className="font-semibold text-amber-300 mb-1">{pick(isDE, "Elite Score", "Elite Score")}</div>
              <p className="text-sm text-zinc-400 mb-3">{pick(isDE, "Erreiche 90+ Punkte", "Reach 90+ points")}</p>
            ...
            <div className="bg-gradient-to-br from-green-900/30 to-gray-800 rounded-xl border border-green-700/50 p-4">
              <div className="font-semibold text-green-300 mb-1">{pick(isDE, "Verbesserung", "Improvement")}</div>
              <p className="text-sm text-zinc-400 mb-3">{pick(isDE, "+20 Punkte in einer Woche", "+20 points in a week")}</p>
              <div className="flex items-center justify-between text-sm">
            ...
            <div className="bg-gradient-to-br from-cyan-900/30 to-gray-800 rounded-xl border border-cyan-700/50 p-4">
              <div className="font-semibold text-cyan-300 mb-1">{pick(isDE, "Hall of Fame", "Hall of Fame")}</div>
              <p className="text-sm text-zinc-400 mb-3">{pick(isDE, "Top 10 erreichen", "Reach Top 10")}</p>
              <div className="flex items-center justify-between text-sm">
        ...
        <div className="bg-gradient-to-r from-cyan-900/40 to-blue-900/40 border border-cyan-700/50 rounded-xl p-6 text-center">
          <h3 className="text-xl font-bold text-cyan-300 mb-2">
            {pick(isDE, "Starte deinen Roast", "Start your roast")}
          </h3>
          <p className="text-sm text-cyan-200/70 mb-4">
            {pick(isDE, "Kostenlos. Sofort verfügbar.", "Free. Available now.")}
          </p>
          <Link
