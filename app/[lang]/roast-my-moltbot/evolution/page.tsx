...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = pick(isDE, "Stack Evolution | ClawGuru", "Stack Evolution | ClawGuru")
  const description = pick(isDE, "Zeige deinen Fortschritt — Before/After Vergleiche deiner Roasts", "Show your progress — Before/After comparisons of your roasts")
  return {
    title,
    description,
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4 text-gray-100">
            {pick(isDE, "Stack Evolution", "Stack Evolution")}
          </h1>
          <p className="text-lg text-gray-300">
            {pick(isDE, "Zeige deinen Fortschritt — Before/After Vergleiche deiner Roasts", "Show your progress — Before/After comparisons of your roasts")}
          </p>
        </div>
        ...
        {!stats || !before || !after ? (
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-12 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-100 mb-2">{pick(isDE, "Keine Evolution-Daten verfügbar", "No evolution data available")}</h2>
            <p className="text-zinc-400">
              {pick(isDE, "Noch keine Roast-Daten für Evolution verfügbar", "No roast data available for evolution yet")}
            </p>
          </div>
        ) : (
          <div className="space-y-6">
            <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
              {/* Stack Name */}
              <h3 className="text-xl font-semibold text-gray-100 mb-4">{pick(isDE, "Community Evolution", "Community Evolution")}</h3>

              {/* Before/After Comparison */}
              <div className="grid grid-cols-2 gap-4 mb-6">
                {/* Before */}
                <div className="bg-red-900/20 rounded-lg p-4 border border-red-800/50">
                  <div className="text-sm text-zinc-500 mb-2">{pick(isDE, "Niedrigster Score", "Lowest Score")}</div>
                  ...
                  <div className="text-sm text-zinc-500">
                    {pick(isDE, "Verbesserungsbedarf", "Needs improvement")}
                  </div>
                </div>
                ...
                <div className="bg-green-900/20 rounded-lg p-4 border border-green-800/50">
                  <div className="text-sm text-zinc-500 mb-2">{pick(isDE, "Höchster Score", "Highest Score")}</div>
                  <div className="flex items-center gap-2 mb-2">
                    <Calendar className="w-4 h-4 text-zinc-500" />
                  ...
                  <div className="text-sm text-zinc-500">
                    {pick(isDE, "Elite Status", "Elite status")}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm text-cyan-400">
                  +{after.score - before.score} {pick(isDE, "Punkte Differenz", "points difference")}
                </span>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  ...
                  )) || <div className="text-zinc-500">{pick(isDE, "Keine Fixes verfügbar", "No fixes available")}</div>}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mt-6 text-center">
          <button className="px-6 py-3 bg-cyan-600 hover:bg-cyan-500 rounded-lg font-semibold text-white transition-colors">
            {pick(isDE, "+ Roast starten", "+ Start roast")}
          </button>
        </div>

        {/* Trust Notice */}
        <div className="bg-amber-900 border-l-4 border-amber-500 p-4 mt-8 text-sm text-amber-100">
          <strong className="text-amber-100">"Not a Pentest" Notice</strong>: {pick(isDE, "Evolution zeigt echte Verbesserungen an eigenen Systemen.", "Evolution shows real improvements on your own systems.")}
        </div>
      </div>
    </div>
  )
