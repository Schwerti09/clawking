...
import { K8sAuditorClient } from "@/components/tools/K8sAuditorClient"
import { getTool } from "@/lib/tools"

const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://clawguru.org"
const SLUG = "k8s-auditor"
...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const title = `${t?.name ?? "K8s Policy Auditor"} — Kubernetes manifest security scanner | ClawGuru`
...
export default function K8sAuditorPage({ params }: { params: { lang: string } }) {
  const locale = (SUPPORTED_LOCALES.includes(params.lang as Locale) ? params.lang : "de") as Locale
  return (
      ...
      <section className="border-b border-white/5 py-14 md:py-16 relative overflow-hidden">
        ...
        <div className="relative container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <h1 className="text-3xl md:text-5xl font-black mb-3 tracking-tight">K8s Policy Auditor ⚓</h1>
            <p className="text-base md:text-xl text-violet-200/80 mb-3">OPA-powered manifest audit.</p>
            <p className="text-sm text-gray-400 max-w-2xl leading-relaxed">
              Paste any Kubernetes manifest (Deployment, Pod, Service, etc.). Run OPA policy checks for RBAC, network policies, resource quotas, Pod security policies, and get specific remediation guidance.
            </p>
          </div>
        </div>
      </section>
