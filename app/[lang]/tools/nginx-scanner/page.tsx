...
import { NginxScannerClient } from "@/components/tools/NginxScannerClient"
import { getTool } from "@/lib/tools"

const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://clawguru.org"
const SLUG = "nginx-scanner"
...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const title = `${t?.name ?? "Nginx Config Scanner"} — Nginx security checker | ClawGuru`
...
export default function NginxScannerPage({ params }: { params: { lang: string } }) {
  const locale = (SUPPORTED_LOCALES.includes(params.lang as Locale) ? params.lang : "de") as Locale
  return (
      ...
      <section className="border-b border-white/5 py-14 md:py-16 relative overflow-hidden">
        ...
        <div className="relative container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <h1 className="text-3xl md:text-5xl font-black mb-3 tracking-tight">Nginx Config Scanner 🕵️</h1>
            <p className="text-base md:text-xl text-emerald-200/80 mb-3">Misconfig detector with explanations.</p>
            <p className="text-sm text-gray-400 max-w-2xl leading-relaxed">
              Paste your nginx config (nginx.conf or server block). Detect SSL/TLS misconfigurations, insecure upstreams, path traversal risks, and missing security headers — with explanations and fixes.
            </p>
          </div>
        </div>
      </section>
