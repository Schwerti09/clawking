...
export default async function OpenTelemetrySecurityPage({
  ...
  const faqSchema = {
    ...
    '@type': 'FAQPage',
    mainEntity: [
      { '@type': 'Question', name: locale === 'de' ? 'Was ist OpenTelemetry und warum ist Security wichtig?' : 'What is OpenTelemetry and why is security important?', acceptedAnswer: { '@type': 'Answer', text: locale === 'de' ? 'OpenTelemetry (OTel) ist ein offener Standard für Observability-Daten (Traces, Metrics, Logs). OTel-Collector hat Zugriff auf alle Telemetriedaten inklusive sensitiver Request-Parameter. Unsicherer Collector = Datenleak. Authentifizierung, TLS und Datenmaskierung sind essentiell.' : 'OpenTelemetry (OTel) is an open standard for observability data (traces, metrics, logs). The OTel Collector has access to all telemetry data including sensitive request parameters. Insecure collector = data leak. Authentication, TLS and data masking are essential.' } },
      { '@type': 'Question', name: locale === 'de' ? 'Wie sichere ich den OpenTelemetry Collector ab?' : 'How do I secure the OpenTelemetry Collector?', acceptedAnswer: { '@type': 'Answer', text: locale === 'de' ? 'OTel Collector Hardening: TLS für alle Receiver/Exporter aktivieren, Auth-Extension (bearertokenauth, basicauth, oidcauth) konfigurieren, Collector nicht direkt ins Internet exponieren, Resource-Limits setzen, sensitiven Daten maskieren mit transform-Processor bevor sie Backends erreichen.' : 'OTel Collector hardening: enable TLS for all receivers/exporters, configure auth extension (bearertokenauth, basicauth, oidcauth), do not expose collector directly to internet, set resource limits, mask sensitive data with transform processor before reaching backends.' } },
      { '@type': 'Question', name: locale === 'de' ? 'Wie maskiere ich sensible Daten in OpenTelemetry Traces?' : 'How do I mask sensitive data in OpenTelemetry traces?', acceptedAnswer: { '@type': 'Answer', text: locale === 'de' ? 'Transform-Processor in der Collector-Config: Attribute löschen (delete_key), ersetzen (set) oder via OTTL-Expressions redact. Beispiel: delete_key(attributes, "http.request.header.authorization"). Span-Attribute wie Passwort-Felder, API-Keys und PII vor dem Export entfernen.' : 'Transform processor in collector config: delete attributes (delete_key), replace (set) or redact via OTTL expressions. Example: delete_key(attributes, "http.request.header.authorization"). Remove span attributes like password fields, API keys and PII before export.' } },
      { '@type': 'Question', name: locale === 'de' ? 'Was ist das Unterschied zwischen OTel und Prometheus für Security?' : 'What is the difference between OTel and Prometheus for security?', acceptedAnswer: { '@type': 'Answer', text: locale === 'de' ? 'Prometheus ist Pull-basiert (scrapet Metriken), OpenTelemetry ist Push-basiert (sendet Daten aktiv). Security-Implikationen: Prometheus benötigt Netzwerkzugang zu allen Services. OTel-Collector als zentrale Aggregationsschicht erlaubt bessere Zugriffskontrolle. OTel unterstützt alle drei Signal-Typen (Traces/Metrics/Logs) mit einheitlicher Auth.' : 'Prometheus is pull-based (scrapes metrics), OpenTelemetry is push-based (actively sends data). Security implications: Prometheus needs network access to all services. OTel Collector as central aggregation layer enables better access control. OTel supports all three signal types (traces/metrics/logs) with unified auth.' } },
    ],
  }
  return (
