...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = pick(isDE, "State of Stack Security 2026 — ClawGuru Datenanalyse", "State of Stack Security 2026 — ClawGuru Data Analysis")
  const description = pick(isDE, "Wir haben über 10.000 Tech-Stacks analysiert. Das sind die häufigsten Sicherheitslücken in 2026.", "We analyzed over 10,000 tech stacks. These are the most common security vulnerabilities in 2026.")
  return {
    title,
    description,
...
export default async function SecurityReport2026Page({ params }: PageProps) {
    ...
    itemListElement: [
      {
        "@type": "ListItem",
        position: 1,
        name: pick(isDE, "Home", "Home"),
        item: `${SITE_URL}/${locale}`,
      },
      {
        "@type": "ListItem",
        position: 2,
        name: pick(isDE, "State of Stack Security 2026", "State of Stack Security 2026"),
  ...
  const articleJsonLd = {
    "@context": "https://schema.org",
    "@type": "Article",
    headline: pick(isDE, "State of Stack Security 2026 — Analyse von 10.000+ Tech-Stacks", "State of Stack Security 2026 — Analysis of 10,000+ Tech Stacks"),
    description: pick(isDE, "Wir haben über 10.000 Tech-Stacks analysiert. Das sind die häufigsten Sicherheitslücken in 2026.", "We analyzed over 10,000 tech stacks. These are the most common security vulnerabilities in 2026."),
    author: {
      "@type": "Organization",
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4 text-gray-100">
            {pick(isDE, "State of Stack Security 2026 — Analyse von 10.000+ Tech-Stacks", "State of Stack Security 2026 — Analysis of 10,000+ Tech Stacks")}
          </h1>
          <p className="text-lg text-gray-300 mb-4">
            {pick(isDE, "Wir haben über 10.000 Tech-Stacks analysiert. Das sind die häufigsten Sicherheitslücken in 2026.", "We analyzed over 10,000 tech stacks. These are the most common security vulnerabilities in 2026.")}
          </p>
        ...
        <div className="bg-amber-900 border-l-4 border-amber-500 p-4 mb-8 text-sm text-amber-100">
          <strong className="text-amber-100">"Not a Pentest" Notice</strong>: {pick(isDE, "Dieser Report dient zur Härtung eigener Systeme. Keine Angriffswerkzeuge. Alle Daten basieren auf freiwilligen Security-Checks.", "This report is for hardening your own systems. No attack tools. All data is based on voluntary security checks.")}
        </div>
        ...
        {!data ? (
          ...
          <>
            ...
            <section className="mb-10">
              <h2 className="text-2xl font-semibold mb-4 text-gray-100">{pick(isDE, "Kern-Erkenntnisse", "Key Findings")}</h2>
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
                  <div className="flex items-center gap-2 mb-2">
                    <Database className="w-5 h-5 text-cyan-400" />
                    <h3 className="font-bold text-gray-100">{pick(isDE, "Analysierte Stacks", "Stacks Analyzed")}</h3>
                  </div>
                  <div className="text-3xl font-bold text-cyan-400">{data.totalStacks.toLocaleString()}</div>
                  <div className="text-sm text-gray-400">{pick(isDE, "Tech-Stacks insgesamt", "Total tech stacks")}</div>
                </div>
                <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
                  <div className="flex items-center gap-2 mb-2">
                    <TrendingUp className="w-5 h-5 text-cyan-400" />
                    <h3 className="font-bold text-gray-100">{pick(isDE, "Ø Security Score", "Avg Security Score")}</h3>
                  </div>
                ...
                <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="font-bold text-gray-100">{pick(isDE, "Kritisches Risiko", "Critical Risk")}</h3>
                  </div>
                  <div className="text-3xl font-bold text-red-400">{data.criticalPercentage.toFixed(1)}%</div>
                  <div className="text-sm text-gray-400">{pick(isDE, "Score < 50", "Score < 50")}</div>
                </div>
                <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
                  <div className="flex items-center gap-2 mb-2">
                    <Shield className="w-5 h-5 text-cyan-400" />
                    <h3 className="font-bold text-gray-100">{pick(isDE, "Häufigste Lücke", "Top Vulnerability")}</h3>
                  </div>
                  <div className="text-lg font-bold text-cyan-400">
                    {data.mostCommonVulnerability || pick(isDE, "N/A", "N/A")}
                  </div>
                  <div className="text-sm text-gray-400">{pick(isDE, "Bei kritischen Stacks", "In critical stacks")}</div>
                </div>
              </div>
            </section>
            ...
            {/* Top Technologies */}
            <section className="mb-10">
              <h2 className="text-2xl font-semibold mb-4 text-gray-100">{pick(isDE, "Top 3 Technologien", "Top 3 Technologies")}</h2>
              <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
                {data.topTechnologies.length > 0 ? (
                  ...
                  <p className="text-sm text-gray-400">{pick(isDE, "Keine Daten verfügbar", "No data available")}</p>
                )}
              </div>
            </section>
            ...
            {/* Methodology */}
            <section className="mb-10">
              <h2 className="text-2xl font-semibold mb-4 text-gray-100">{pick(isDE, "Methodik", "Methodology")}</h2>
              <div className="bg-gray-800 p-6 rounded-lg border border-gray-700 space-y-4">
                <div className="flex items-start gap-3">
                  <Calendar className="w-5 h-5 text-cyan-400 flex-shrink-0 mt-1" />
                  <div>
                    <h3 className="font-semibold text-gray-100 mb-1">{pick(isDE, "Zeitraum", "Timeframe")}</h3>
                    <p className="text-sm text-gray-300">
                      {data.oldestDate && data.newestDate
                        ...
                        : pick(isDE, "Daten werden gesammelt", "Data is being collected")}
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-100 mb-1">{pick(isDE, "Stichprobe", "Sample Size")}</h3>
                    <p className="text-sm text-gray-300">
                      {isDE
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-100 mb-1">{pick(isDE, "Score-Berechnung", "Score Calculation")}</h3>
                    <p className="text-sm text-gray-300">
                      {pick(isDE, "Der Security Score (0-100) basiert auf HTTP-Header-Analyse, TLS-Konfiguration, Security-Header-Prüfung und Best-Practice-Compliance. Höhere Werte bedeuten bessere Security-Hygiene.", "The security score (0-100) is based on HTTP header analysis, TLS configuration, security header review, and best-practice compliance. Higher values indicate better security hygiene.")}
                    </p>
                  </div>
                </div>
                <div className="bg-amber-900 border-l-4 border-amber-500 p-4 text-sm text-amber-100">
                  <strong className="text-amber-100">{pick(isDE, "Wichtiger Hinweis", "Important Note")}:</strong>{" "}
                  {pick(isDE, "Dieser Report ist KEIN Penetrationstest. Alle Daten basieren auf automatisierten Security-Scans, die freiwillig von Entwicklern durchgeführt wurden. Keine Angriffswerkzeuge, keine illegalen Aktivitäten.", "This report is NOT a penetration test. All data is based on automated security scans voluntarily performed by developers. No attack tools, no illegal activities.")}
                </div>
              </div>
            </section>
            ...
            <section className="mb-10">
              <div className="bg-gradient-to-r from-cyan-900 to-blue-900 p-8 rounded-lg border border-cyan-700 text-center">
                <h2 className="text-2xl font-bold mb-4 text-gray-100">
                  {pick(isDE, "Analysiere deinen Stack", "Analyze Your Stack")}
                </h2>
                <p className="text-lg text-gray-300 mb-6">
                  {pick(isDE, "Erhalte einen detaillierten Security-Score und konkrete Verbesserungsvorschläge für deinen Tech-Stack.", "Get a detailed security score and concrete improvement recommendations for your tech stack.")}
                </p>
                <Link
            ...
            <section className="mb-10">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <Link href={`/${locale}/roast-statistics`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
                  ...
                  <div className="text-sm text-gray-300">{pick(isDE, "Live-Daten und Trends", "Live data and trends")}</div>
                </Link>
                <Link href={`/${locale}/runbooks`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
                  <div className="font-semibold text-cyan-400">{pick(isDE, "Security Runbooks", "Security Runbooks")}</div>
                  <div className="text-sm text-gray-300">{pick(isDE, "600+ ausführbare Playbooks", "600+ executable playbooks")}</div>
                </Link>
                <Link href={`/${locale}/securitycheck`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
                  ...
                  <div className="text-sm text-gray-300">{pick(isDE, "HTTP-Header Analyse", "HTTP header analysis")}</div>
                </Link>
                <Link href={`/${locale}/openclaw`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
                  <div className="font-semibold text-cyan-400">{pick(isDE, "OpenClaw Framework", "OpenClaw Framework")}</div>
