...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = isDE
    ? "Security für FinTech — DORA, PSD2, BaFin, MaRisk | ClawGuru"
    : "Security for FinTech — DORA, PSD2, BaFin, MaRisk | ClawGuru"
  const description = isDE
    ? "Security-Compliance für FinTechs und regulierte Finanzdienstleister. DORA-Readiness, PSD2-SCA, BaFin-Audit, MaRisk-Controls. Strategy Call kostenlos."
    : "Security compliance for FinTechs and regulated financial services. DORA readiness, PSD2-SCA, BaFin audit, MaRisk controls. Free strategy call."
  return {
    title,
    description,
...
const getPains = (isDE: boolean) => [
  {
    icon: Scale,
    title: isDE ? "DORA-Deadline läuft" : "DORA deadline is running",
    desc: isDE
  ...
  {
    icon: AlertTriangle,
    title: isDE ? "BaFin-Audit = Millionenrisiko" : "BaFin audit = millions at risk",
    desc: isDE
  ...
  {
    icon: Lock,
    title: isDE ? "PSD2-SCA ist kein Einmal-Projekt" : "PSD2-SCA is not a one-off project",
    desc: isDE
  ...
  {
    icon: TrendingUp,
    title: isDE ? "Sub-Processor-Kaskade außer Kontrolle" : "Sub-processor cascade out of control",
    desc: isDE
...
const getSolutions = (isDE: boolean) => [
  {
    icon: FileCheck,
    title: isDE ? "DORA-Compliance-Package" : "DORA compliance package",
    desc: isDE
  ...
  {
    icon: Shield,
    title: isDE ? "BaFin/MaRisk/BAIT-Controls" : "BaFin/MaRisk/BAIT controls",
    desc: isDE
  ...
  {
    icon: Lock,
    title: isDE ? "PSD2-SCA Monitoring & Updates" : "PSD2-SCA monitoring & updates",
    desc: isDE
  ...
  {
    icon: Building2,
    title: isDE ? "Deutsche Regulator-Erfahrung" : "German regulator experience",
    desc: isDE
...
const getRegulations = (isDE: boolean) => [
  { name: "DORA", desc: isDE ? "Digital Operational Resilience Act — EU-weit" : "Digital Operational Resilience Act — EU-wide" },
  { name: "PSD2", desc: isDE ? "Payment Services Directive — SCA, TPP-Access" : "Payment Services Directive — SCA, TPP access" },
  { name: "MaRisk", desc: isDE ? "BaFin Mindestanforderungen an das Risikomanagement" : "BaFin minimum requirements for risk management" },
  { name: "BAIT", desc: isDE ? "Bankaufsichtliche Anforderungen an die IT" : "Banking supervisory requirements for IT" },
  { name: "KAIT", desc: isDE ? "Kapitalverwaltungsaufsichtliche Anforderungen IT" : "Capital-management supervisory IT requirements" },
  { name: "ISO 27001", desc: isDE ? "Information Security Management System" : "Information security management system" },
]
...
export default function ForFintechPage({ params }: PageProps) {
  ...
  const articleSchema = buildAuthoredArticleSchema({
    headline: isDE
      ? "Security für FinTech — DORA, PSD2, BaFin, MaRisk"
      : "Security for FinTech — DORA, PSD2, BaFin, MaRisk",
    description: isDE
      ? "Security-Compliance für regulierte Finanzdienstleister. DORA-Readiness, PSD2-SCA, BaFin-Audit-Vorbereitung."
      : "Security compliance for regulated financial services. DORA readiness, PSD2-SCA, BaFin audit preparation.",
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        <nav className="text-sm text-gray-500 mb-6" aria-label="Breadcrumb">
          <ol className="flex flex-wrap items-center gap-2">
            ...
            <li>/</li>
            <li className="text-gray-300">{isDE ? "Für FinTech" : "For FinTech"}</li>
          </ol>
        </nav>
        ...
        <section className="mb-14">
          <div className="inline-block mb-4 px-3 py-1 rounded-full text-xs font-semibold border border-amber-500/30 bg-amber-500/10 text-amber-300">
            {isDE ? "Vertikal: FinTech & regulierte Finanzdienstleister" : "Vertical: FinTech & regulated financial services"}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <BookingButton
              type="audit"
              label={isDE ? "DORA-Readiness Scoping (kostenlos)" : "Free DORA readiness scoping"}
              locale={locale}
              source="for_fintech_hero_primary"
              variant="primary"
              subject={isDE ? "DORA-Readiness FinTech" : "DORA readiness FinTech"}
            />
            <Link
              href={`/${locale}/check`}
          ...
          <div className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
            ...
            <span>✓ {isDE ? "Deutsche Regulator-Erfahrung" : "German regulator experience"}</span>
            <span>✓ {isDE ? "Audit-ready Dokumentation" : "Audit-ready documentation"}</span>
          </div>
        </section>
        ...
        <section className="mb-14">
          <div className="text-xs font-mono uppercase tracking-widest mb-3 text-amber-400">
            {isDE ? "Abgedeckte Regulierungen" : "Covered regulations"}
          </div>
        ...
        <section className="mb-14">
          <div className="text-xs font-mono uppercase tracking-widest mb-2 text-red-400">
            {isDE ? "FinTech-Realität 2026" : "FinTech reality 2026"}
          </div>
          <h2 className="text-3xl font-bold text-white mb-6">
            {isDE ? "Warum dein CTO nachts wach liegt" : "Why your CTO isn't sleeping"}
          </h2>
        ...
        <section className="mb-14">
          <div className="text-xs font-mono uppercase tracking-widest mb-2 text-amber-400">
            {isDE ? "Mit ClawGuru FinTech-Pack" : "With ClawGuru FinTech pack"}
          </div>
          <h2 className="text-3xl font-bold text-white mb-6">
            {isDE ? "Regulator-ready in Monaten, nicht Jahren" : "Regulator-ready in months, not years"}
          </h2>
          <div className="grid md:grid-cols-2 gap-4">
            {solutions.map((s, i) => {
              const Icon = s.icon
        ...
        <section className="mb-14">
          <div className="text-xs font-mono uppercase tracking-widest mb-2 text-amber-400">
            {isDE ? "Engagement-Modell" : "Engagement model"}
          </div>
          <h2 className="text-3xl font-bold text-white mb-6">
            {isDE ? "FinTech Security Engagement" : "FinTech Security Engagement"}
          </h2>
          <div className="bg-gradient-to-br from-amber-900/20 to-cyan-900/10 border-2 border-amber-500/40 rounded-2xl p-8 max-w-3xl">
            <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
              <div>
                <div className="text-sm text-amber-400 font-semibold">{isDE ? "6-Monate-Retainer" : "6-month retainer"}</div>
                <h3 className="text-2xl font-bold text-white mt-1">
                  {isDE ? "DORA + MaRisk Full-Stack" : "DORA + MaRisk Full-Stack"}
                </h3>
              </div>
              <div className="text-right">
                <div className="text-3xl font-black text-amber-300">{isDE ? "ab €45.000" : "from €45,000"}</div>
                <div className="text-xs text-gray-400">{isDE ? "Fixed Fee" : "fixed fee"}</div>
              </div>
            </div>
            <ul className="space-y-2 text-sm text-gray-300 mb-6">
              <li className="flex items-start gap-2"><Check className="h-4 w-4 text-amber-400 mt-0.5 shrink-0" />{isDE ? "DORA ICT-Risk-Framework komplett dokumentiert" : "DORA ICT risk framework fully documented"}</li>
              <li className="flex items-start gap-2"><Check className="h-4 w-4 text-amber-400 mt-0.5 shrink-0" />{isDE ? "MaRisk AT 7.2 + BAIT-Controls implementiert" : "MaRisk AT 7.2 + BAIT controls implemented"}</li>
              <li className="flex items-start gap-2"><Check className="h-4 w-4 text-amber-400 mt-0.5 shrink-0" />{isDE ? "PSD2-SCA Monitoring & Update-Loop" : "PSD2-SCA monitoring & update loop"}</li>
              <li className="flex items-start gap-2"><Check className="h-4 w-4 text-amber-400 mt-0.5 shrink-0" />{isDE ? "ICT-Third-Party-Register + Konzentrationsrisiko-Analyse" : "ICT third-party register + concentration-risk analysis"}</li>
              <li className="flex items-start gap-2"><Check className="h-4 w-4 text-amber-400 mt-0.5 shrink-0" />{isDE ? "Incident-Classification & Reporting-Playbook" : "Incident classification & reporting playbook"}</li>
              <li className="flex items-start gap-2"><Check className="h-4 w-4 text-amber-400 mt-0.5 shrink-0" />{isDE ? "TLPT-Scoping-Dokument" : "TLPT scoping document"}</li>
              <li className="flex items-start gap-2"><Check className="h-4 w-4 text-amber-400 mt-0.5 shrink-0" />{isDE ? "BaFin-Prüfungs-Vorbereitung (Dry-Run)" : "BaFin examination prep (dry run)"}</li>
              <li className="flex items-start gap-2"><Check className="h-4 w-4 text-amber-400 mt-0.5 shrink-0" />{isDE ? "Monatliche Office-Hours mit SecOps- und Compliance-Experten" : "Monthly office hours with SecOps and compliance experts"}</li>
            </ul>
            <BookingButton
              type="audit"
              label={isDE ? "DORA-Scoping Call buchen" : "Book DORA scoping call"}
              locale={locale}
              source="for_fintech_pricing"
              variant="primary"
              subject={isDE ? "FinTech DORA + MaRisk Engagement" : "FinTech DORA + MaRisk engagement"}
              className="w-full justify-center"
            />
            <p className="text-xs text-gray-500 text-center mt-3">
              {isDE ? "Enterprise-Projekt (Bank, BaFin-Volllizenz)? " : "Enterprise project (bank, full BaFin license)? "}
        ...
        <section className="mb-14">
          <div className="bg-gradient-to-r from-amber-900/30 to-cyan-900/30 border border-amber-700/50 rounded-2xl p-10 text-center">
            <h2 className="text-3xl font-bold text-white mb-3">
              {isDE ? "DORA-Deadline tickt. Starten wir?" : "DORA deadline is ticking. Shall we start?"}
            </h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <BookingButton
                type="audit"
                label={isDE ? "DORA-Scoping buchen" : "Book DORA scoping"}
                locale={locale}
