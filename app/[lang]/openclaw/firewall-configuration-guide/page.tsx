...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://clawguru.org/${lang}/openclaw/firewall-configuration-guide`, languages: Object.fromEntries(LANGS.map(l => [l, `https://clawguru.org/${l}/openclaw/firewall-configuration-guide`])) },
    robots: 'index, follow',
  };
}

export default function OpenClawFirewallPage({ params }: PageProps) {
  const { lang } = params;
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-8 text-sm">
          <strong>"Not a Pentest" Trust-Anker</strong>: Firewall-Konfiguration schützt eigene Server. Keine Angriffswerkzeuge.
        </div>
        <h1 className="text-4xl font-bold mb-4">OpenClaw Firewall Konfiguration</h1>
        <p className="text-lg text-gray-600 mb-8">Minimale Angriffsfläche durch striktes Whitelist-Prinzip — nur explizit erlaubte Verbindungen werden durchgelassen.</p>

        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4">🔥 UFW Basis-Konfiguration</h2>
...
ufw status numbered`}</pre>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4">📊 Port-Status Matrix</h2>
            ...
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {[
                  ...
                  <tr key={port} className="border-b hover:bg-gray-50">
                    <td className="p-3 font-mono text-sm">{port}</td>
                    <td className="p-3">{service}</td>
                    <td className="p-3">{status}</td>
                    <td className="p-3 text-sm text-gray-600">{access}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            <a href="/securitycheck" className="block bg-gray-50 p-4 rounded-lg hover:bg-gray-100"><div className="font-semibold text-blue-600">🛡️ Security Check</div><div className="text-sm text-gray-600">Port Scan</div></a>
            <a href="/runbooks" className="block bg-gray-50 p-4 rounded-lg hover:bg-gray-100"><div className="font-semibold text-blue-600">📚 Firewall Runbooks</div><div className="text-sm text-gray-600">UFW Guides</div></a>
