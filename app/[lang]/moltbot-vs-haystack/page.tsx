...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  const title = isDE
    ? "Moltbot vs Haystack: Sicheres AI-Agent-Framework vs NLP-Pipeline | ClawGuru"
    : "Moltbot vs Haystack: Secure AI Agent Framework vs NLP Pipeline | ClawGuru"
  const description = isDE
    ? "Moltbot vs Haystack Vergleich: Security-First AI-Agent-Orchestrierung vs flexibler NLP-Pipeline-Builder. Prompt Injection Defense, RBAC, RAG-Security und Self-Hosted-Betrieb im Direktvergleich."
    : "Moltbot vs Haystack comparison: security-first AI agent orchestration vs flexible NLP pipeline builder. Prompt injection defense, RBAC, RAG security and self-hosted operation in direct comparison."
  return {
    title, description,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-xl text-cyan-400 mb-4">{isDE ? "Security-First-Orchestrierung vs flexible NLP-Pipeline" : "Security-First Orchestration vs Flexible NLP Pipeline"}</p>
        <p className="text-lg text-gray-300 mb-8">
          {isDE
            ? "Haystack ist hervorragend für RAG und NLP-Pipelines — aber ohne eingebaute Sicherheitskontrollen. Moltbot bringt Prompt-Injection-Defense, RBAC, Audit-Logging und Multi-Tenant-Isolation out-of-the-box."
            : "Haystack excels at RAG and NLP pipelines — but without built-in security controls. Moltbot delivers prompt injection defense, RBAC, audit logging and multi-tenant isolation out of the box."}
        </p>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{isDE ? "Wann welches Framework?" : "When to Use Which Framework?"}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-cyan-900 border border-cyan-700 p-4 rounded-lg">
              <h3 className="font-bold text-cyan-300 mb-2">{isDE ? "Moltbot wenn…" : "Moltbot when…"}</h3>
              <ul className="text-sm text-cyan-200 space-y-1">
                <li>▸ {isDE ? "Sicherheit ist nicht verhandelbar" : "Security is non-negotiable"}</li>
                <li>▸ {isDE ? "Multi-Tenant AI SaaS-Produkt" : "Multi-tenant AI SaaS product"}</li>
                <li>▸ {isDE ? "Regulierte Industrie (HIPAA, PCI, ISO)" : "Regulated industry (HIPAA, PCI, ISO)"}</li>
                <li>▸ {isDE ? "AI-Agenten mit Tool-Zugriff" : "AI agents with tool access"}</li>
                <li>▸ {isDE ? "Compliance-Evidenz erforderlich" : "Compliance evidence required"}</li>
              </ul>
            </div>
            <div className="bg-gray-800 border border-gray-700 p-4 rounded-lg">
              <h3 className="font-bold text-gray-300 mb-2">{isDE ? "Haystack wenn…" : "Haystack when…"}</h3>
              <ul className="text-sm text-gray-400 space-y-1">
                <li>▸ {isDE ? "Maximale Pipeline-Flexibilität gefragt" : "Maximum pipeline flexibility needed"}</li>
                <li>▸ {isDE ? "Reines RAG/Dokumentensuche-Projekt" : "Pure RAG/document search project"}</li>
                <li>▸ {isDE ? "Data-Science-Team, kein DevSecOps" : "Data science team, no DevSecOps"}</li>
                <li>▸ {isDE ? "Prototyp oder interne Nutzung" : "Prototype or internal use"}</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-gray-100">{isDE ? "Weitere Vergleiche" : "More Comparisons"}</h2>
          <div className="grid grid-cols-2 gap-4">
            <a href={`/${locale}/moltbot-vs-langchain`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "Ähnlicher Framework-Vergleich" : "Similar framework comparison"}</div>
            </a>
            <a href={`/${locale}/moltbot-vs-semantic-kernel`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "Microsoft vs Security-First" : "Microsoft vs security-first"}</div>
            </a>
            <a href={`/${locale}/moltbot-vs-llamaindex`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "RAG-Framework-Vergleich" : "RAG framework comparison"}</div>
            </a>
            <a href={`/${locale}/moltbot/agentic-rag-security`} className="block bg-gray-800 p-4 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
              ...
              <div className="text-sm text-gray-300">{isDE ? "RAG sicher implementieren" : "Implement RAG securely"}</div>
            </a>
          </div>
        </section>
      </div>
