...
export async function generateMetadata(props: { params: { lang: string } }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: buildLocalizedAlternates(locale, "/moltbot-hardening"),
    openGraph: {
      images: ["/og-image.png"], title: copy.title, description: copy.description, type: "website" },
  }
}

export default function MoltbotHardeningPage(props: { params: { lang: string } }) {
  const locale = (SUPPORTED_LOCALES.includes(props.params.lang as Locale) ? props.params.lang : "de") as Locale
  const copy = getMoltbotCopy(locale)
      ...
      <Container>
        <div className="mx-auto max-w-4xl space-y-8">
          <header className="space-y-4">
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-cyan-400/90">Moltbot Hardening</p>
