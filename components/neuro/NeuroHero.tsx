...
export default function NeuroHero({ prefix = "", dict }: { prefix?: string; dict?: any }) {
  ...
  useEffect(() => {
    function onSel(e: Event) {
      ...
      const n = Number((ce as any).detail?.selected)
      if (!isNaN(n)) setSelectedCount(n)
    }
    function onFound(e: Event) {
      const ce = e as CustomEvent
      const n = Number((ce as any).detail?.found)
    ...
    <section className="relative py-14 text-center">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-3xl sm:text-5xl font-extrabold bg-gradient-to-r from-white to-cyan-300 bg-clip-text text-transparent">
          {dict?.hero_title || "Neuro – Dein persönlicher Runbook‑Guide"}
        </h1>
        <p className="mt-4 text-gray-300 text-base sm:text-lg">
          {dict?.hero_subline || "Gib deinen Technologie‑Stack an – ClawGuru findet die wichtigsten Runbooks, priorisiert nach Relevanz und gibt dir einen klaren Plan."}
        </p>
        <div className="mt-4 flex items-center justify-center gap-3 text-sm">
          <span className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-gray-300">
            {(dict?.metrics_tags_selected || "Tags ausgewählt") + ": "}{selectedCount}
          </span>
          <span className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-gray-300">
            {(dict?.metrics_recommendations || "Empfehlungen (live)") + ": "}{foundCount == null ? "—" : foundCount}
          </span>
        </div>
        <div className="mt-6">
