...
export default function PracticeExamClient({ locale }: Props) {
        ...
        <div className="bg-gradient-to-br from-cyan-900/20 to-purple-900/20 border border-cyan-700/40 rounded-2xl p-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <h2 className="text-2xl font-bold text-white">
                {pick(isDE, "Defender Foundation — Practice", "Defender Foundation — Practice")}
              </h2>
            </div>
          </div>
          <p className="text-gray-300 mb-6">
            {pick(isDE, "15 Praxis-Fragen aus dem echten Foundation-Fragenpool. Du bekommst Score + Erklärung je Frage + ein personalisiertes Feedback, welche Themen du vertiefen solltest.", "15 practice questions drawn from the actual Foundation question pool. You get a score + per-question explanation + personalized feedback on which topics to deepen.")}
          </p>
          <div className="grid sm:grid-cols-3 gap-3 mb-6 text-sm">
            <div className="bg-black/30 rounded-lg p-3 text-center">
              ...
              <div className="text-xs text-gray-400">{pick(isDE, "Fragen", "Questions")}</div>
            </div>
            <div className="bg-black/30 rounded-lg p-3 text-center">
              <div className="text-xl font-bold text-cyan-300">{config.durationMinutes} min</div>
              <div className="text-xs text-gray-400">{pick(isDE, "Zeit", "Time")}</div>
            </div>
            <div className="bg-black/30 rounded-lg p-3 text-center">
              <div className="text-xl font-bold text-cyan-300">{config.passingScorePercent}%</div>
              <div className="text-xs text-gray-400">{pick(isDE, "Bestanden", "Pass")}</div>
            </div>
          </div>
          <button
            onClick={start}
          ...
          >
            ...
            {pick(isDE, "Prüfung starten", "Start exam")}
            <ArrowRight className="h-5 w-5" aria-hidden />
          </button>
          <p className="mt-3 text-xs text-gray-500">
            {pick(isDE, "Kein Account nötig · Resume bei Tab-Schließen · DSGVO-konform (Daten bleiben im Browser)", "No account needed · Resume on tab-close · GDPR-compliant (data stays in browser)")}
          </p>
        </div>
      </div>
    )
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4 text-sm">
          <div className="text-gray-400">
            {pick(isDE, "Frage", "Question")} <span className="text-white font-bold">{currentIdx + 1}</span> /{" "}
            {questions.length}
          </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 sm:p-8 mb-4">
          ...
          {multi && (
            <p className="mt-4 text-xs text-gray-500">
              {pick(isDE, "Mehrere Antworten möglich", "Multiple answers possible")}
            </p>
          )}
        </div>
  ...
  const resultHeadline = passed
    ? pick(isDE, "Bestanden! 🛡️", "Passed! 🛡️")
    : pick(isDE, "Nicht bestanden — aber nah dran.", "Not passed — but close.")
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className={`bg-gradient-to-br ${passed ? "from-cyan-900/30 to-green-900/20 border-cyan-700/50" : "from-amber-900/20 to-red-900/10 border-amber-700/40"} border rounded-2xl p-8 mb-6 text-center`}>
        <div className="inline-flex items-center gap-2 mb-3 px-3 py-1 rounded-full bg-black/40 text-xs font-semibold text-gray-300">
          {pick(isDE, "Ergebnis", "Result")} · {config.certificateLabel}
        </div>
        <div className={`text-6xl font-black mb-2 ${passed ? "text-cyan-300" : "text-amber-300"}`}>
          {score.percent}%
        </div>
        <div className="text-sm text-gray-400 mb-4">
          {score.correct} / {score.total} {pick(isDE, "korrekt", "correct")} ·{" "}
          {pick(isDE, "Bestanden bei", "Pass at")} {config.passingScorePercent}%
        </div>
        <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4">{resultHeadline}</h2>
        {passed ? (
          <p className="text-gray-300 mb-6 max-w-xl mx-auto">
            {pick(isDE, "Solide Foundation-Basis! Für das echte Zertifikat (60 Fragen, offiziell) setz dich auf die Warteliste — Early-Bird gibt 20% Rabatt.", "Solid Foundation basis! For the real certificate (60 questions, official), join the waitlist — early-bird gets 20% off.")}
          </p>
        ) : (
          <p className="text-gray-300 mb-6 max-w-xl mx-auto">
            {pick(isDE, "Du bist dicht am Pass-Score. Unsere Runbook-Library deckt alle Prüfungs-Topics ab — mit Day Pass bekommst du 24h Vollzugriff.", "You're close to the pass score. Our runbook library covers every exam topic — with a Day Pass you get 24h full access.")}
          </p>
        )}
            ...
            <>
              <a
                href={`mailto:certification@clawguru.org?subject=${encodeURIComponent(
                  pick(isDE, "Defender Foundation Warteliste (Practice bestanden)", "Defender Foundation waitlist (passed practice)")
              ...
              >
                ...
                {pick(isDE, "Warteliste — Early Bird 20%", "Waitlist — early-bird 20% off")}
              </a>
              <a
              ...
              >
                ...
                {pick(isDE, "Ergebnis teilen", "Share result")}
              </a>
            </>
          ) : (
            <>
              ...
              >
                ...
                {pick(isDE, "Day Pass €5 — Runbooks unlocken", "Day Pass €5 — unlock runbooks")}
              </Link>
              <button
                onClick={start}
              ...
              >
                ...
                {pick(isDE, "Nochmal versuchen", "Try again")}
              </button>
            </>
          )}
        </div>
      ...
      {weakTopics.length > 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 mb-6">
          <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-3">
            {pick(isDE, "Fokus-Themen für dich", "Focus topics for you")}
          </h3>
          ...
          <p className="text-xs text-gray-500 mt-3">
            {pick(isDE, "Diese Themen deckt unsere Runbook-Library mit hunderten Copy-Paste-Playbooks ab.", "Our runbook library covers these topics with hundreds of copy-paste playbooks.")}
          </p>
        </div>
      )}
      ...
      <details className="bg-gray-900 border border-gray-800 rounded-2xl p-6 group">
        <summary className="font-semibold text-gray-100 cursor-pointer list-none flex items-center justify-between">
          <span>{pick(isDE, "Alle Fragen + Erklärungen ansehen", "Review all questions + explanations")}</span>
