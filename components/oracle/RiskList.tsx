...
export default function RiskList({ risks, prefix = "", dict }: { risks: any; prefix?: string; dict?: any }) {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
        ...
        {risks?.timeline && (
          <p className="text-sm text-gray-400 mt-2">{(dict?.timeline_prefix || 'Erste kritische Bedrohung erwartet ab: ')}{risks.timeline}</p>
        )}
      </div>

      {items.length > 0 ? (
        <>
          <h3 className="text-xl font-semibold mt-6">{dict?.critical_risks_title || 'Kritische Risiken'}</h3>
            ...
            <div key={idx} className="bg-white/5 rounded-xl p-4 border border-white/10 hover:border-cyan-500/30 transition">
              <div className="flex justify-between items-start flex-wrap gap-2">
                ...
                <span className="text-sm bg-cyan-500/20 text-cyan-300 px-2 py-1 rounded-full">
                  {(dict?.probability_label || 'Wahrscheinlichkeit')} {risk.probability}%
                </span>
              </div>
              ...
              {risk.recommended_runbook && (
                <div className="mt-3">
                  <Link href={`${prefix}/runbook/${risk.recommended_runbook}`} className="text-sm text-cyan-400 hover:text-cyan-300">
                    {(dict?.recommended_runbook_link || 'Empfohlenes Runbook →')} {risk.recommended_runbook}
                  </Link>
                </div>
              )}
            </div>
        ...
        <div className="text-center text-gray-400 py-12">{dict?.empty_state || 'Keine akuten Risiken für den gewählten Scope erkannt. 🛡️'}</div>
      )}
    </div>
  )
}
